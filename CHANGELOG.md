...
- **b2 consistency re-run: all six audit tranches now share the standard cross-vendor two-pass design (+49 water-only flags).**  Batch b2's original research pass had run without web access (its verdicts were replaced in-session by a single-vendor fallback, documented as a deviation); the batch was re-run end-to-end under the standard design — valid Codex web research over all 393 pairs (handoff validator: 0 content problems) → Sonnet 5 adversarial judgment.  The re-run reproduced the shipped disposition on 324/393 pairs with **zero removals** (the maintainer's map review confirmed all 8 challenged flags; all five earlier human overrides came back true).  Its 69-row triage shipped **49 additional water-only flags**: 40 maintainer-accepted and 9 resolved by a human-delegated **Fable 5 final arbitration** — the eight pre-2021 Latvia/Lithuania vintage pairs (adjudicated on the shipped WB geometry itself) and the two `Area under National Administration` polygons, identified from their WB adjacency signatures as the Malawian and Mozambican **Lake Malawi/Niassa water-surface units** (1954 median-line boundary, IBS No. 112); one inter-run arbiter disagreement was settled by a deterministic re-measurement tie-break (frozen in `b2_rerun_arbiter_tiebreak.json`).  Every accepted pair passed the four-layer bridge pipeline (49 pairs: 32 bridged / 17 not; the province-polygon layer corrected one off-border citation — the Kerio crossing — to the on-reach Kolowa Rd–Rorok Rd bridge, WB polygon test PASS).  Count effects: water-only **698 → 747** (352/395), ADM1 moderate **8,088 → 8,071**, stringent **7,768 → 7,719**; ADM0 roll-ups **27 → 28** — MOZ↔TZA completes as all-water (bridged, so ADM0 moderate stays **320**; stringent 299 → **298**).  All frozen verdicts under `build_data/water_screen_rebuild/verdicts/` (`b2_rerun*.json`); the original `b2.json` history is untouched.
