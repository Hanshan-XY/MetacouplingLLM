...
- **The deterministic rejection rule dropped: the 269 edges it closed adjudicated by the two models, none water-only; the cross-type union at every ladder width (no data change).**  The July hybrid auto-reject rule closed, without any model looking, an edge nominated only by a wider Natural Earth river rung (5 to 20 km) whose share at every layer's base width (Natural Earth rivers 2.5 km, lakes 125 m, HydroRIVERS and HydroLAKES 500 m) is below 0.20. It held for 269 edges on the current screen record, and it could miss a border that only the wider rungs see: the Orinoco border Anzoátegui↔Bolívar, shipped water-only, was first nominated at 10 km and reaches 0.24 at best at a base width (HydroRIVERS). Maintainer: "Adjudicate all 269 (Recommended)". The 269 (207 domestic, 62 cross-border; none with a two-model verdict, checked against the campaigns' research inputs rather than the July audit queue, which lists the rule's rows too) went through the standing two-pass: blind GPT-5.6 Sol research (269 of 269 valid; water-only on none) and Claude Sonnet 5 adversarial judgment (269 agents; water-only on none); gate A 0 / B 0 / C 22 / D 247, maintainer: "No override". The completeness check no longer lets the rule's ledger close a nomination: every nominated candidate carries a two-model verdict. At the maintainer's request the cross-type union takes the widest rung of each Natural Earth ladder (rivers 20 km, lakes 1,500 m; HydroRIVERS and HydroLAKES 500 m; bar 0.80), so it combines the layers at every width the ladders measure: union nominations 1,144 → 2,045 (accepted among them 723 → 744), one new candidate (Plužine↔Šavnik, already adjudicated, not water-only), edges nominated by any screen 3,285 → 3,286; attributability 803/803. No shipped row changes. Specification and record: `build_data/water_screen_rebuild/rule_rejections/` (`SPEC_mr1_rule_rejections.md`, `run_screens_mr1.py`, `build_queue_mr1.py`, the gate, `mr1_rulings.json`).

- **The build's stages: every water-only row applied in Stage 3 from one reviewed file, the denylist in Stage 4 (no edge, flag or count change).**  The stages are the order in which the build applies its inputs, and the water-only rows sat in three stores: the 298 rows of the first classification in `build_data/bridge_classified_authoritative.csv`, which the geometry build read in Stage 3, and the 481 + 24 rows added from the 2026-07 water screens onward in `rescreen_water_overlay_pairs.csv` and `rescreen_gap_overlay_pairs.csv`, which the correction engine applied in Stage 4.  The split recorded when rows were found, not a difference in method: every row is nominated by the current screens, adjudicated by the two-model design and flagged by the same four-layer crossing pipeline.  The five contacts that the water adjudication found not to be borders were a constant of the build script, applied in Stage 1.  Maintainer: "I prefer the option B, and please don't forget to move the 5 pairs to stage 4, also change the the repository docs."; after a reviewer's-view discussion of a three-stage alternative, "Keep option B".  The stages are now: (1) geometry — exact contact, the source-relabel and the unit merge, 8,422 edges; (2) de facto — +16, 8,438 edges, written by `scripts/build_pericoupling_db.py` with the five denylisted contacts still among them; (3) water — `scripts/apply_overlays.py` applies `data/water_classification_pairs.csv` (803 rows), flagging 779 existing edges water-only and adding the 24 water borders between units that do not touch (column `adds_edge`; maintainer: "Stage 3 (Recommended)"), 8,462; (4) edge corrections — +4 land-gap borders, 8,466, then −5 contacts listed with their evidence and ruling in `data/denylist_pairs.csv`, 8,461.  The ADM0 roll-up (26 pairs) is computed once, after Stage 4.  The water file carries for each row the codes, names and ISO codes, `water_type`, `water_body`, `has_bridge`, `adds_edge`, `border_km` (the 24 added rows only), `adjudication`, `verification_tier` and `source` (the provenance record, verbatim; for the 298 first-round rows the old base file's note followed by its crossings where recorded); the old base file's country names and its first-round `border_km` and `ne_cov` are not carried.  Deleted (git history keeps them): `build_data/bridge_classified_authoritative.csv` (maintainer: "Delete it (Recommended)"), `rescreen_gap_overlay_pairs.csv` and `rescreen_water_overlay_pairs.csv`.  Code: `build_pericoupling_db.py` runs Stages 1–2 only (`--bridge-csv`, `write_water_separated_manifest` and `_ADM1_FALSE_POSITIVE_DENYLIST` removed); `apply_overlays.py` runs Stages 3–4 and stops on a denylisted pair that is also a water or land-gap row; `build_all.py` pins only the four GeoPackages, and `--full` stops if a denylisted contact is absent from the geometry output; `draw_validation_sample.py` reads the water file (the validation sample's population holds the same 239 rows, so the draw is unchanged).  Shipped data: the edge list has the same 8,461 rows with the same values, its last 28 rows reordered (the 24 water borders now precede the 4 land borders); the water table has the same rows in the same order, and its `note` column now reads "water-only border on a shared edge (edge screens; two-model adjudication)" (779 rows) or "water-only border between non-touching units (corridor census; two-model adjudication)" (24 rows) (maintainer: "Rewrite without 'rebuild' (Recommended)"); every count is unchanged (moderate 8,065, stringent 7,658; 803 = 407 / 396; ADM0 326 / 320 / 300; tiers A 268 / B 238 / C 297 — tier B stays the preregistered validation study's frame).  A full regeneration from the pinned GeoPackages (`build_all.py --full`, 12/12 counts; the geometry step wrote 8,438 edges with every denylisted contact present) reproduces the four shipped files byte for byte, and the country matrix and the de facto manifest are unchanged (`build_data/stage_order/st1_full_regeneration_compare.txt`).  "Rebuild" is removed from the prose of METHODS, PROVENANCE, REPRODUCING and the script docstrings (maintainer: "I prefer prose only."); folder names, older entries of this file and the per-row `source` records keep it.  Specification and record: `build_data/stage_order/` (`SPEC_st1_stages.md`, `make_st1_files.py`, `compare_full_regeneration.py`).

- **The disputed areas' de facto administrators taken from Natural Earth; three province pairs added (ADM1 8,458 → 8,461; moderate 8,062 → 8,065; stringent 7,655 → 7,658).**  The World Bank's disputed-area (NDLSA) layer names no administrator for its 24 tracts (`SOVEREIGN` empty, `WB_STATUS` "Non-determined legal status area"), and the build's table `_NDLSA_TRACT_ADMIN` was authored: it left six tracts unassigned (three with no single administrator — No Man's Land, Abyei, the UN Buffer Zone — and the three island tracts, which touch no unit), and it assigned the tract named Karakoram Range, which is the Shaksgam Valley, to Pakistan.  The maintainer asked for one rule for all tracts ("Why not assigned them? ... why not using unified rules?"; "assigning the 3 island and look into the Karakoram tract.").  Each tract is now assigned, whole, to the administrator that Natural Earth v5.1.2 (the latest release, 2022-05-13; `ne_10m_admin_0_disputed_areas` first, `ne_10m_admin_0_countries` for the rest of the tract; zip SHA-256 in `PROVENANCE.md`) records for the largest part of it; a part outside every Natural Earth polygon is left out of the count, a dependency counts for its sovereign, and a tract whose largest part has no administrator the World Bank lists as a country stays unassigned.  There are no exceptions (maintainer: "Remove the exception (pure majority)"): Western Sahara goes to Morocco whole although Natural Earth records its eastern third (33.5%) as self-administered, Jammu and Kashmir to India whole (the Siachen Glacier, 1.9%, is recorded with claims only), the Ilemi Triangle to Kenya whole (a 17 km² area inside it, 0.5%, recorded as South Sudanese).  Result: 23 of the 24 tracts assigned; only the UN Buffer Zone is not (Natural Earth: UN zone or Northern Cyprus for 89%).  Changed: Karakoram Range Pakistan → China ("Admin. by China; Ceded to China by Pakistan; Claimed by India"); Lapthal and Shipki Pass China → India (83.5% and 54.5%); No Man's Land → Israel; Abyei → Sudan; British Indian Ocean Territory, the Falklands and South Georgia & the South Sandwich Islands → the United Kingdom (no pair).  Kauirik stays with China by majority (57.0%; Natural Earth's "Samdu Valleys" record, administered by India, covers 42.1%; maintainer: "I agree with the the majority rule, including Kauirik staying with China").  The build's check now requires every assigned tract to touch its administrator's territory (its own polygon or another tract assigned to it) unless the tract touches no unit at all (the islands), in place of the check that one tract per administrator touches it; a restored country border is measured between the two countries' de facto territories (maintainer: "Between de facto territories (Recommended)"), so China–Pakistan is 455.1 km (491.2 km with the Karakoram Range on Pakistan's side; measured against China's standard polygon it would be 219.9 km, leaving out the Karakoram crest between Gilgit-Baltistan and the Shaksgam Valley); a country row keeps listing all its administrator's tracts (maintainer: "All the administrator's tracts"; Israel–Syria gains "No Man's Land", 79.1 km); Morocco–Mauritania is unchanged.  The province lists of the tracts whose administrator changed are that country's World Bank provinces touching the tract (Lapthal IND035, Shipki Pass IND014, Karakoram Range CHN028, No Man's Land ISR001/ISR003, Abyei SDN012/SDN013; none for the islands).  Measured before the change with the build's own derivation and adopted by the maintainer ("Adopt all (Recommended)"): three province pairs join the de facto overlay — Jerusalem (ISR003)↔Ramallah (PSE013) across the Latrun no-man's-land, 12.4 km; Southern Darfur (SDN012)↔Warrap (SSD010), 47.8 km, and Southern Kordofan (SDN013)↔Warrap, 34.2 km, across Abyei; none leaves.  Data: `pericoupled_adm1_edge_list.csv` +3 rows (8,461; 3,374 regions, 196 countries); `disputed_overlay_pairs.csv` 16 → 19 rows (3 country + 16 province pairs; the China–Pakistan length and the Israel–Syria label); the ADM0 matrix unchanged (326 / 320 / 300); the strict view (`de_facto_borders=False`) unchanged (8,445); no water row changes (803 = 407 / 396; a de facto edge has no border arc).  The corridor census takes only pairs that are not edges: the three pairs leave its population (23,410 → 23,407), and Jerusalem↔Ramallah, nominated by its short-gap presence rule (a 422 m gap) and rejected in the nt2 two-pass as a dry gap, leaves its nominations (243 → 242).  Build: `_NDLSA_TRACT_ADMIN`, `_NDLSA_TRACT_ADM1` and `derive_disputed_overlay` in `scripts/build_pericoupling_db.py`; `build_all.py` expects 8,461 / 8,065 / 7,658; tests `test_tract_administrators_follow_natural_earth` and `test_overlay_pairs_across_abyei_and_latrun`; `docs/ndlsa_tract_audit.csv` regenerated with Natural Earth's record and share for each tract (`build_data/ndlsa_ne/make_tract_audit.py`).  A full rebuild from the pinned GeoPackages (`build_all.py --full`, 12/12 counts) differs from the shipped files by exactly these rows; `PeriTelecoupling_clean.csv` and `water_separated_pairs.csv` are byte-identical, and the correction layer is a byte-stable no-op on the new data (`build_data/ndlsa_ne/da1_full_rebuild_compare.txt`).  The mapping describes de facto administration for a connectivity dataset; it is not a sovereignty claim.  Specification and record: `build_data/ndlsa_ne/` (`SPEC_da1_defacto_administrators.md`, `ne_admin_of_tracts.*`, `measure_ne_overlay.*`).

- **The crossing screen's width measured on the ground; one crossing added (water-only 803 = 407 / 396; moderate 8,061 → 8,062).**  Layer 1 of the fixed-crossing pipeline counted an OpenStreetMap bridge way when it intersected both units' polygons buffered by 0.0012° — a default of the June classifier, 133 m north–south but 133·cos φ m east–west (55 m on the northernmost shipped row).  It now counts a way that comes within 100 m of both units' polygons, measured on the ground (an azimuthal-equidistant frame centred on the way; likewise the tunnel, dam-top, dyke and embankment ways of bridged rows).  No mapping standard sets such a width; a read-only dry run over the 803 rows (26,831 bridge ways measured) set it: the screen's disagreement with the reviewed flags is flat from 25 to 250 m (100–108 rows) and rises on both sides, the bridged rows whose nearest bridge way falls short of a unit fall short by a median of 94 m, and of the round values tested 100 m reopens the fewest settled rows.  The dry run also found that the 2026-09-15 screen had reported no bridge way on four rows whose ways existed unedited: its Overpass client accepted any HTTP-200 answer, and Overpass reports a query that ran out of time inside such an answer.  The client now counts an answer only if it reports no error (on the re-run it rejected one, Illinois↔Iowa, 0 elements), the search reaches every way within 1 km of the border, and the location test locates its two distances on the ground (no outcome changes on the 89 located rechecks on record).  Re-run over all 803 rows (OpenStreetMap as of 2026-09-24) and validated way by way against the dry run, the screen agrees with the shipped flag on 700 rows; 97 of the 103 others already carry an adversarial recheck; 7 went to the GPT-5.6 Sol recheck (the 6 without one, plus Panamá↔Panamá Oeste under the way-class rule applied to the new record, its trunk-road way 117 m from one unit): 6 confirmed their flags, and Gambela↔Pibor Administrative Area across the Akobo was upgraded on a maintainer ruling — the Raad Bailey road bridge (100 m, built in 2010; WFP Logistics Cluster), 16 m from the shared border.  Data: `build_data/bridge_classified_authoritative.csv` 1 flag (base 105 / 193) + a `cw1` clause on its 3 rechecked rows, SHA-256 re-pinned (`d898f739…6f649d5`, LF-normalised) in `build_all.py`, `PROVENANCE.md`, `REPRODUCING.md`; `rescreen_water_overlay_pairs.csv` a `cw1` clause on 4 rows; `water_separated_pairs.csv` the base flag; `build_all.py` EXPECTED moved; `build_all.py` OK (byte-stable no-op); a `--full` rebuild from the pinned GeoPackages reproduces the four files it builds (the ADM1 edge list, the ADM0 matrix, the disputed overlay and the water table) byte for byte.  Docs: METHODS (`has_bridge` paragraph, a `cw1` paragraph, current counts), PROVENANCE, REPRODUCING (the layer-1 rule and its basis, expected counts), BRIDGE_CLASSIFICATION_METHODOLOGY status note, INTRODUCTION.  Record: `build_data/water_screen_rebuild/crossing_width/` (specification `SPEC_cw1_crossing_width.md`, tracked like the ba1 and gd1 specifications; `cw1_rulings.json`, `cw1_maintainer_rulings_2026-09-24.txt`).
- **Every screen distance measured on the ground; the corridor census within 100 km on the ground (no data change).**  The edge screens measured every distance on the ellipsoid but located the nearest point of a feature in plain degrees, which overstates the distance to an obliquely running feature (by up to 88–106% at 60–70° on a 13,372-pair test), and the corridor census chose its candidates within 0.9° (100.2·cos φ km east–west).  The nearest point is now located in a local frame in which a metre east equals a metre north (beyond 2.5 km, in an azimuthal-equidistant frame centred on the point) and measured with `GEOD.inv`; the border arc's facing test measures its width and reciprocity the same way and no longer lets rounding decide its midpoint test (a chord shorter than 1 m is contact and stays in the arc; a point within 2×10⁻⁷° of a stretch already in the arc only repeats it); the corridor census reads the build's polygons and takes every pair within 100 km of each other on the ground, 23,410 pairs (20,653 before), measuring each pair in an azimuthal-equidistant frame (a pair the build's polygons join at a single point while the World Bank polygons keep it apart, two pairs, is measured on the World Bank polygons).  Edge screens: 3,285 edges nominated (3,248), 43 new, 6 no longer, every shipped row still nominated (779/779); census: 243 pairs nominated (245), all 24 shipped rows again.  The 44 nominations without a two-model record (29 new edge nominations, 4 auto-reject ledger rows whose rule no longer holds, 11 census pairs) went through the standard two-pass: GPT-5.6 Sol research and Sonnet-5 adversarial judgment found water-only on none, gate A 0 / B 0 / C 8 / D 36, maintainer: no override.  **No shipped row changes** (edges 8,458; water-only 803 = 406/397; moderate 8,061; stringent 7,655); completeness 0 open in every band, attributability 803/803.  `docs/METHODS_adjacency.md` (§2.5, §8 and a `gd1` section), `docs/REPRODUCING.md` and `src/metacouplingllm/data/PROVENANCE.md` describe the rule; specification and record in `build_data/geodesic_distances/` (`SPEC_gd1_geodesic_distances.md`, whose change log lists every implementation defect and maintainer decision).
- **CHANGELOG, METHODS and PROVENANCE checked against the data at each commit (docs only).**  Every database entry of this file was compared with the shipped data at the commit that made the change and at its parent (36 data commits, `build_data/doc_checks/changelog_counts_by_commit.py`): every headline count agrees.  Six statements were corrected: the source-relabel moves eleven strips of ten host polygons (the `sr1` entry said ten strips); the rg1 fold removed two edges, not three; the nine arbitrated b2 rows are seven of the eight Latvia/Lithuania pairs and the two placeholder contacts; batch b2's first 76 flags split 57 domestic / 19 cross-border (not 65 / 25); the 17 hydro-water rows shipped 5 bridged / 12 not (6 / 12 once the Uruguay River was folded in); and the disputed-areas layer's sovereignty field is empty for every tract, not absent.  METHODS and PROVENANCE said all 715 creek-band edges had been set aside on 2026-07-10: 714 were, and the 715th was an edge no screen population held.  No data change.

- **Database documentation checked against the code and the shipped data (docs only).**  Two full reads of METHODS, PROVENANCE, REPRODUCING and the database parts of INTRODUCTION and MANUAL, each procedural sentence traced to the script that implements it and each figure recomputed from the shipped data or measured on the build's polygons.  Stage 3 reads only the reviewed bridge classification (the build reads no Natural Earth layer); contiguity is exact contact, and `SNAP_TOL_DEG` (the disputed-tract and relabel-owner touch tests) plays no part in it; REPRODUCING's correction layer is three manifests of 509 rows (28 edge-restoring, 481 flags) and its census description is the 2026-09-10 rule; the median inter-vertex spacing is about 5.4×10⁻³° (~600 m), not 3.9×10⁻⁴°; the sub-tolerance offsets re-measured on the build's polygons (Anguilla 1.3 m, Ethiopia–Sudan 31 m, the Dominican pair 39 m; the Namibia–Zimbabwe ADM0 gap ~135 m); counts brought to the shipped data (four land-gap rows, 505 overlay rows, five denylist entries, two lake-only country pairs, six roll-ups completed by the rescreen-water overlay, an attributability enumeration summing to 803).  No data change.  (#132)

- **One border arc for every edge screen; one row returns to the water-only set and three leave it (water-only 805 → 803 = 406 / 397; moderate 8,061 unchanged; stringent 7,653 → 7,655).**  The edge screens had each defined the shared border their own way.  The Natural Earth ladders sampled the exact shared line unmerged (a median of 70 pieces per edge, each with its own three or more points), so a share was weighted by how finely the line happened to be cut.  The HydroRIVERS and HydroLAKES screens sampled one unit's outline within 10⁻³° (about 111 m) of the other, a band that added 4,932 km beyond the exact line, most of it third-unit borders at the tripoints and coastline, though also 273 edges' river channels that the Ocean Mask cuts out as sea.  All of them read the raw World Bank polygons, which the build relabels and merges, so Kajiado↔Kilimanjaro and Narok↔Mara had never been screened, and the records rounded shares to two decimals.  Campaign `ba1` replaces them with one rule, frozen in a specification before any measurement (maintainer approval 2026-09-21): the screens read the build's own polygons; an edge's **border arc** is the exact shared line with its pieces merged, the stretches where the polygons overlap, the 5×10⁻⁴° band of the four land-gap pairs, and **facing stretches** where the two outlines face each other across a gap of at most 1,000 m (not on or across a third unit's outline, reciprocal, the chord's midpoint inside neither unit); each piece is sampled at equal geodesic intervals shorter than 500 m; shares are exact.  The four screens keep their widths and bars and are joined by a **combined river** screen (within 2.5 km of a Natural Earth river or 500 m of a HydroRIVERS reach; bar 0.50), a **combined lake** screen (125 m of a Natural Earth lake or 500 m of a HydroLAKES polygon; 0.40) and a **cross-type union** (0.80).  Measured blind: 3,248 edges nominated (3,257 before); 75 new nominations, 61 of them without a two-model record; three shipped rows nominated by no screen; two rows that left the set on 2026-09-21 nominated again.  The maintainer's map check of a 96-row sample found nothing to correct.  The 63 pairs went through the standard two-pass (blind GPT-5.6 Sol research, Sonnet-5 adversarial judgment; gate A 0 / B 2 / C 16 / D 45) and the maintainer ruled the two disagreements after a gap measurement and a reading of the cited sources: **Lekoumou↔Niari water-only** (it returns, with no fixed crossing: the bridges the research named lie inside Niari on the World Bank map) and **Garissa↔Wajir mixed** (not water-only).  The three unnominated rows (Chiba↔Tokyo on the Edo River, Montevideo↔San José on the Santa Lucía, Haiphong↔Quảng Ninh on the Đá Bạch–Bạch Đằng) leave the water-only set under the removal rule set with the specification; the edges stay, and `docs/FUTURE_WORK.md` §4 lists them.  Hai Duong↔Quang Ninh, re-adjudicated, stays out.  Counts: water-only 803 = 406 with a fixed crossing / 397 without; river 689 (394 / 295); rescreen-water 481 (296 / 185); creek-band rows 50 (45 / 5); large-river rows 14 (11 / 3); tier A 268; moderate 8,061; stringent 7,655; lenient 8,458 and every ADM0 count unchanged (all four rows are domestic).  `ba1_screens.csv` is now the edge screens' record: completeness 0 open; whole-graph attributability 803/803 (779 by the screens on their border arc, 24 non-touching rows by the corridor census).  `build_all` EXPECTED, the `apply_overlays` docstring and `tests/test_apply_overlays.py` updated; METHODS, PROVENANCE, REPRODUCING, FUTURE_WORK.  Record: `build_data/arc_and_combined_screens/` and `build_data/water_screen_rebuild/` (`border_arc.py`, `build_arc_cache.py`, `run_screens_ba1.py`, `ba1_screens.csv`, `ba1_report.txt`).

- **HydroRIVERS screens take the sampling rule of the other screens; three unnominated rows leave the water-only set (water-only 808 → 805 = 408 / 397; moderate 8,059 → 8,061; stringent 7,650 → 7,653).**  The three HydroRIVERS-family screens (domestic sweep, cross-border sweep — which also computes the cross-border HydroLAKES share — and creek-band enrichment) set the number of samples on each part of the shared border from its coordinate length — one per 4.5×10⁻³°, about 500 m along a north–south line but only about 500·cos(latitude) m along an east–west one — so how many points a border received depended on its latitude and orientation; the Natural Earth screens and the domestic HydroLAKES sweep already set it from the geodesic length (⌊L/500 m⌋ + 1 intervals, at least 2).  On the maintainer's decision (campaign `gs1`) all three now use that rule.  Both rules space the points evenly along each part in coordinates, so the average spacing is below 500 m while single intervals vary with direction and latitude (median 493 m, 5th–95th percentile 403–545 m on the shipped borders).  Re-run on the pinned inputs, the change reshuffles bar-edge cases in both directions (median coverage change about zero).  **Nineteen creek-band pairs were newly nominated** with no adjudication record (17 domestic, 2 cross-border) and went through the standard two-pass — blind GPT-5.6 Sol research, Sonnet-5 adversarial judgment, gate A 0 / B 1 / C 6 / D 12 — and the maintainer ruled the one disagreement (Vitebsk↔Zilupes novads) not water-only after a gap measurement: all 19 rejected.  **Three shipped rows were no longer nominated by any screen** (Lekoumou↔Niari 0.50 → 0.49; Kyegegwa↔Ssembabule 0.50 → 0.33 on a 0.65 km border; Hai Duong↔Quang Ninh 0.51 → 0.49 — creek-band rows ruled water-only on maps on 2026-09-14) and, on the maintainer's decision, leave the water-only set: the database is complete relative to its screens, and a row no screen nominates falls outside the method.  That is **not a finding that they are land**; the edges stay as ordinary land borders and the three are recorded as known omissions in `docs/FUTURE_WORK.md` §4.  Counts: water-only 805 = 408 with a fixed crossing / 397 without; river 691 (396 / 295); rescreen-water 483 (298 / 185); creek-band rows 51 (47 / 4); tier A 270; moderate 8,061; stringent 7,653; lenient 8,458 and every ADM0 count unchanged (all three are domestic).  Completeness 0 open; whole-graph attributability 805/805.  Also fixed in the working archive: the creek-band enrichment script, which a later comment edit had left unparseable.  Also corrected in METHODS: the HydroLAKES rung's nomination total counted two sweep rows that are not edges (293 → 291 on the edge list).  Record: `build_data/geodesic_sampling/`.

- **The source-relabel attributions re-adjudicated under the current two-vendor design; no data change (campaign `sr1`, 2026-09-20).**  Stage 1 reassigns eleven reviewed sliver-corridor strips of ten host polygons to the unit that administers the land before contiguity is measured, on the strength of an audit of 2026-07-04: a research pass and an adversarial refutation pass over the 14 strips the shape screen flags on cross-country borders, unanimous.  That audit predates the GPT-5.6 Sol research route (the first Codex handoff is 2026-07-10), its workflow script and output were not kept, and the session that ran it is long gone, so **no model was on record for either pass** — the one adjudicated input of the database without a model-labelled record.  The shape screen was re-run on the pinned GeoPackage (144 candidates, 14 touching a cross-country border across 13 hosts, identical to the frozen scan) and all 14 strips went through the standard pass, blind to the shipped treatment: a GPT-5.6 Sol research pass, then a Sonnet-5 adversarial judgment pass with no repository access.  **Both passes confirmed every attribution, owner codes included** — 11 strips to a same-country neighbour (Arusha's two ribbons to Mara and Kilimanjaro, Gedo to Lower Juba, Wajir to Garissa, Galgaduud to Mudug, Lamwo to Kitgum, Salta to Jujuy, ʿAsīr to Jizan, Atyrau to West Kazakhstan, Braničevo to Bor, Béchar to Tindouf) and 3 kept as the host's own territory (the Vennbahn treaty corridor, the Courantyne west-bank strip, the Dhekelia link corridor).  Gate K 12 / C 2 / B 0 / F 0; the two C rows are the judgment pass's requests for a human glance, closed by the maintainer the same day.  No flag, no manifest row and no count changes; METHODS, PROVENANCE, both drafts and the manuscript now name the models behind the attributions.  Record: `build_data/sliver_readjudication/`.

- **Way-class follow-up of the crossing pipeline: one fixed-crossing flag removed (water-only 808 = 409 / 399; moderate 8,060 → 8,059).**  Layer 1 of the four-layer `has_bridge` pipeline accepts any OpenStreetMap way tagged as a bridge on a road, path or railway; the rule that only a road or rail crossing counts is applied by the later layers, and a row on which the screen and the web verification agreed never reached the adversarial recheck, the one layer that states the footbridge exclusion.  The screen had kept way names, not classes, so a diagnostic re-query (campaign `wc1`, 2026-09-20; OpenStreetMap as of that date) recorded the class of every qualifying way for the 354 bridged rows with a layer-1 "bridge way found": 4,923 ways, 899 of them foot or cycle ways; **340 rows have a road or rail way in both units**, 6 have foot or cycle ways only, 5 a service road, farm track or access-restricted way only, 3 a disused railway or a bridge outline only.  Of those 14, 11 rest on a road or rail structure named by the web verification or an earlier recheck, or on a maintainer ruling (Puente Arce, the Pļaviņas dam-top road, the Rāmnieki bridge, the Grevelingendam, the H-03 bridge at Zhvanets, Portumna Bridge, the Saltinho bridge, the PY19 bridge, the Aghband–Kalaleh bridge, the Spiersbachbrücke, the Margara bridge).  The 3 with no such structure on record went to the GPT-5.6 Sol adversarial recheck: Sankt Gallen↔Gamprin was confirmed (the Haag–Bendern road bridge, 370 m from the farther unit) and two downgrades were proposed.  Maintainer rulings of 2026-09-20: **Basel-Landschaft↔Baden-Württemberg (`CHE004<->DEU001`) loses its flag** — its only crossing is the foot and cycle way over the Augst–Wyhlen barrage — and Telšiai↔Vaiņodes novads keeps its flag under a convention now stated in METHODS: a structurally complete railway bridge on a disused line with its track in place counts, because the line can reopen (the logic already applied to politically closed borders).  Counts: water-only 808 = **409** with a fixed crossing / **399** without (was 410 / 398), river rows 397 / 297, base file 104 / 194; ADM1 moderate **8,059** (was 8,060); lenient 8,458, stringent 7,650 and every ADM0 count unchanged.  Base CSV re-pinned (`b95d5f08…c986cff`, LF-normalised); a `wc1` clause on the three rechecked rows' provenance.  Record: `build_data/water_screen_rebuild/bridge_unification/` (`bridge_way_class_*`, `wc1_*`).

- **REPRODUCING: the OpenStreetMap bridge screen described as coded (docs only).**  The screens list said the bridge screen takes a "road/rail bridge, causeway, dam-top road, or tunnel … (ferries and footbridges excluded)".  The query is `way[highway][bridge]`, `way[railway][bridge]` and `man_made=bridge`, and the tag rule drops only ways under construction or proposed, so a bridge on a footpath or cycleway passes it; the rule that only road or rail crossings count is applied by the later layers (web verification, adversarial recheck, maintainer rulings).  Found while sizing how many bridged rows could rest on a foot-only way; a way-class diagnostic over the 354 bridged rows with a layer-1 "yes" was run the same day (`build_data/water_screen_rebuild/bridge_unification/bridge_way_class_*`; 340 have a road or rail way in both units, and the remaining 14 are being reconciled against their verification records).  No data change.

- **METHODS / PROVENANCE: two July statements made exact (docs only).**  (1) The HydroLAKES cross-check was summarised as "15 cross-border lake candidates, 3 rejected as mixed, 12 shipped", which mixes two candidate sets and does not add up.  The archived records give **19** candidates: the first sweep flagged 27 borders, 15 of them not yet water-only (11 accepted; 2 mixed, Migori↔Mara and Diffa↔Lac; 2 left as they were, both Lake Malawi contacts of the water-surface units, accepted by the later full-ladder audit), and the geodesic 500 m re-run nominated 4 more (the Dead Sea accepted; the Burundi–Rwanda inter-lake connector and two Norway–Sweden segments rejected as mixed); 12 shipped = 11 + 1.  (2) The discharge-scaled-ladder diagnostic said its 26 added non-water edges were adjudicated "22 from its audit queue, 4 on frozen verdicts"; the four (`SSD002<->UGA002`, `NAM001<->ZMB110`, `HUN008<->SVK002`, `AFG013<->TKM003`) were re-adjudicated under the two-model design on 2026-09-01 with every other earlier rejection, so none rests on a frozen verdict.  Found by re-deriving the papers' July figures from the archived campaign records (`build_data/_archive_pre_rebuild/premeasure/flags_adjudication.json`, `build_data/geodesic_rescreen/REPORT.md`, `build_data/water_screen_rebuild/rejection_unification/rj{1,2}_rulings.json`); every other July figure checked was confirmed.  No data change.

- **One corridor-census nomination had no two-model record; re-adjudicated, rejection upheld (no data change).**  A code-level audit of the papers found that Vorarlberg↔Vaduz (`AUT008<->LIE011`; gap 492 m, short-corridor hit) — one of the 245 nominations of the 2026-09-10 corridor census — carried only a near-miss-era verdict and the *mechanical* closure of 2026-09-01 ("no current screen nominates this pair"), a premise that stopped holding when the re-derived census nominated it nine days later.  It was never queued for the census campaigns because it sat in the predecessor's file and was counted as already adjudicated, and the completeness check had hidden it by counting the mechanical closure as a record (it no longer does).  Campaign `nt4`, 2026-09-20, standard cross-vendor two-pass, blind: GPT-5.6 Sol research and Sonnet-5 adversarial judgment both returned not water-only at high confidence (the units are not adjacent; the gap is dry alpine ridge), so no maintainer ruling was needed.  METHODS now states the re-adjudication of the 201 earlier rejections exactly (199 on 2026-09-01; Cahul↔Vaslui on 2026-07-22; this pair on 2026-09-20) and carries a campaign paragraph.  Completeness: 0 open in every band.  Record: `build_data/water_screen_rebuild/corridor_census_v2/` (`nt4_*`).

- **METHODS: three stale passages corrected (docs only).**  (1) §2.4 and §4 still listed *five* genuine sub-tolerance borders, with the placeholder–Malawi lake corner shipping as a land-gap row; it was reversed on 2026-07-18 as a point contact, the shipped manifest has **four** rows, and the band therefore holds four genuine borders and three artifacts.  (2) The mid-lake paragraph of §8 said the rescreen-water manifest carries "the twelve HydroLAKES-nominated" lake reclassifications and the rescreen-gap overlay "the one" non-touching lake restoration; the shipped files carry **51** lake-class rows and **three** non-touching lake borders (Skadar, Tonlé Sap, Lac de Biaufond).  (3) The corridor-census description now states the rule as implemented: the 0.80 test is over *samples* and is the lake share plus the river share (a capped sum, a generous stand-in for a union); the wide variant nominates on lakes at 1,500 m plus big reaches at 2,500 m; the population is a planar 0.9° pre-filter followed by a geodesic gap of at most 100.2 km, existing edges excluded; and the bar-sensitivity statement covers all 24 shipped recoveries (re-checked 2026-09-20).  Found by auditing the manuscript's database section statement by statement against the code and the data.  No data change.
...
- **HydroLAKES bar lowered from 0.5 to 0.40, the Natural Earth lake bar; the newly nominated band adjudicated (no data change).**  The two lake rungs had carried different bars for no recorded reason: the NE lake bar of 0.40 has a derivation (bimodal coverage, bar in the empty valley just under the weakest genuine NE-captured lake border at 0.48; `build_data/lake_screen_analysis/lake_ladders.py`), while the HydroLAKES bar had only been lowered from 0.6 to 0.5 in July to match the river bars.  The band [0.40, 0.50) holds 55 edges: 14 ship water-only, every one also nominated by a river rung; 20 had been rejected by earlier audits; **21 were new** and were adjudicated by the standard cross-vendor two-pass (GPT-5.6 Sol research: 20 not water-only / 1 water-only; Sonnet-5 adversarial judgment: 21 not water-only) with a maintainer map ruling on the one disagreement (St. Paul↔St. Peter, Antigua — not water-only).  **All 21 rejected: no flag and no count changes.**  METHODS (rebuild parameters; a campaign paragraph), PROVENANCE and REPRODUCING now give the HydroLAKES bar as 0.40.  Record: `build_data/water_screen_rebuild/hydrolakes_band/`.

- **METHODS / PROVENANCE: post-#122 review pass (docs only).**  Two current-state figures had gone stale.  (1) The creek-band rows' crossing split is **48 / 6**, not 47 / 7: the 2026-09-14 campaign shipped 47 / 7 and the crossing unification of 2026-09-16 then located a bridge on Haiphong↔Quang Ninh (Cầu Lại Xuân); the METHODS S4 row and the PROVENANCE manifest table now give both (the campaign paragraphs keep the 47 / 7 they reported).  (2) "34 of the 623 shipped river borders the two hydro sweeps cover" dated from 2026-09-10; live, **673** river rows sit on existing borders (694 river − 21 non-touching) and 34 of them (10 base + 24 rescreen-water) fall below the 0.50 bar at 500 m — the 34 is unchanged because every row added since clears the bar by construction.  No data change.

- **The ten arbitrated verdicts are now maintainer map rulings; the third model no longer stands behind any row (no count changes).**  Ten verdicts of the 2026-07 audit — the pre-2021 Latvia/Lithuania subdivision-vintage pairs and the two lake-surface placeholder contacts — had been delegated by the maintainer, for want of map evidence, to a Fable-5 final-arbitration pass (2026-07-17; nine shipped, one rejected; one inter-run disagreement settled by a re-measurement tie-break).  They were the only verdicts in the database resting on that model.  Campaign `ar1` (2026-09-19): a worksheet on the shipped WB geometry — the existing GPT-5.6 Sol research and Sonnet-5 judgment verdicts (both water-only on all ten), a deterministic overlay of OSM waterways along each arc (50 m sampling; share within 100/250/500 m of a river or stream, longest dry run and its position), the HydroRIVERS/HydroLAKES instrument, and a rendered map per pair — and the maintainer's ruling on each: **all ten match the arbitration** (Telšiai↔Saldus not water-only; nine water-only).  No flag, count or file other than `rescreen_water_overlay_pairs.csv` changes: the nine rows' `source` now cites the maintainer ruling in place of the arbitration clause, and the 2026-09-18 pilot row's `source` no longer names the pilot's two models, so no shipped provenance string names a retired model; tier A is now literally human-verified for the nine.  The adjudication design is two models, one per vendor, plus maintainer map rulings.  METHODS (escalation paragraph; §8 paragraph), PROVENANCE updated.  Both files now also name the two standing adjudication models — OpenAI's GPT-5.6 Sol, run in the Codex app (research pass and crossing recheck), and Anthropic's Claude Sonnet 5 (adversarial judgment) — matching the papers.  **Terminology:** the per-row provenance had paired a product name with a model name (`Codex research + Sonnet-5 adversarial judgment`); both passes are now labelled by their model — `Codex research` → `GPT-5.6 Sol research` (608 occurrences) and `Codex adversarial recheck` → `GPT-5.6 Sol adversarial recheck` (131) in `rescreen_water_overlay_pairs.csv`, `rescreen_gap_overlay_pairs.csv` and `build_data/bridge_classified_authoritative.csv` (text of `source`/`note` only; the base CSV's SHA-256 re-pinned, LF-normalised `a14cd879…0c7ac9b`, in `build_all.py`, PROVENANCE, REPRODUCING), and the same relabelling in METHODS, PROVENANCE, FUTURE_WORK, the bridge methodology note and this file.

- **Pilot re-adjudicated under the cross-vendor design: one existing border reclassified water-only (807 → 808; stringent 7,650; moderate unchanged).**  The nine-pair pilot of the 2026-07 audit queue had been adjudicated by the original audit design — a Sonnet-5 research pass feeding a Fable-5 judgment pass, both on one vendor — before the research pass moved to GPT-5.6 Sol, and was never re-run; all nine were rejected and none shipped, but the statement that every verdict on record carries the cross-vendor design did not hold for them, and `completeness_check.py` hid it by counting membership of `audit_queue.csv` (which lists the pilot) as a cross-vendor record.  Campaign `pr1` (2026-09-18): all nine still nominated by a current screen → GPT-5.6 Sol research, blind (9/9 valid; 8 not water-only, 1 water-only) → Sonnet-5 adversarial judgment (9 agents, one structured return each) → gate A 0 / B 2 / C 4 / D 3 → deterministic gap measurement → maintainer map rulings on the two disagreements: **Kapisa↔Parwan accepted** along the Panjshir River (longest uncovered run 1.0 km of 97 km), Kuçovë↔Lushnjë upheld as land; eight rejections stand.  The accepted pair passed the four-layer bridge pipeline without a recheck (unified OSM screen: 7 open bridge ways in both units; research and judge agree; trunk-road bridge on the shared arc) and ships as a tier-A row of `rescreen_water_overlay_pairs.csv` (485 → 486).  Counts: water-only ADM1 807 → **808** (409/398 → **410/398**), river 693 → 694, tier A 272 → 273, stringent 7,651 → **7,650**; edges 8,458, moderate 8,060 and ADM0 326/320/300 unchanged; attributability 808/808, completeness 0 open.  `build_all.py` EXPECTED and `tests/test_apply_overlays.py` moved; METHODS (§8 paragraph), PROVENANCE, REPRODUCING updated.  The completeness check now excludes the pilot from the audit-queue record and counts it only through the frozen `pr1` rulings.

- **Overlay engine: canonical row order; the committed edge list and water table re-sorted once (moves only, no row or count changes).**  A clean `--full` rebuild had reproduced the shipped edge list and water table row for row but not in the committed order: the engine appended overlay rows at the end (`append-only`), so the committed files carried the order of the project's append history (drift from edge 8,431 and water row 299 onwards, surfaced by the bu1 rebuild check). `scripts/apply_overlays.py` now writes both files in canonical order — native/base rows in build order, verbatim, then the overlay rows in registry order and manifest row order — the order a fresh rebuild produces; running it once re-sorted the committed files (6 edge lines and 53 water lines moved; both files set-identical to before). Verified: `--check` byte-stable no-op, `build_all.py` counts unchanged (8,458 / 8,060 / 7,651; water 807 = 409/398), and a clean `--full` rebuild from the pinned GeoPackages now reproduces `pericoupled_adm1_edge_list.csv`, `water_separated_pairs.csv` and `PeriTelecoupling_clean.csv` byte for byte. `REPRODUCING.md` byte-identity caveat updated accordingly.

- **Crossing flags unified: the four-layer fixed-crossing pipeline re-run over all 807 water-only rows; 14 flags changed on maintainer rulings (bridged 403 → 409, unbridged 404 → 398; moderate 8,054 → 8,060; campaign bu1, 2026-09-16).**  Water-only status had been unified (ru1/wu1/wu2) but the crossing flags came from three generations — the June base pipeline (OSM classifier, ~130 m unit buffer + 400 m sweep; 57 huge-lake rows never queried), the rebuild-era four-layer runs (a faithful July copy; a 50 m/300 m September fork; location tolerances ~3 km / 1.7 km / 500 m), and the folded pre-rebuild rows with no screen or location record (45 rescreen-water, 6 rescreen-gap, 3 shore contacts).  Layer 1 (`bu1_screen.py`: the shipped classifier's query and tag rules, 130 m in-both-units test, corridor stand-in for non-touching pairs, dense 500 m sweep on borders wider than 0.5 deg²) answered 807/807: agree 691, no bridge way for 54 bridged rows, a bridge way for 56 unbridged rows, tunnel/dam-top only for 6.  Those 116 + 15 bridged rows without a location record → GPT-5.6 Sol adversarial recheck (88 yes / 43 no; structure + coordinates + open-to-traffic + in-both-units) → location test (≤ 500 m of the shared arc or ≤ 750 m of both units; 750 m–3 km flagged; 2 wrong-unit citations caught: Yacyretá, Queen Louise) → 27 proposed flips → maintainer map rulings (verbatim `bu1_maintainer_rulings_2026-09-16.txt`): 10 of 20 upgrades and 4 of 7 downgrades accepted, 13 flags kept.  Data: `rescreen_water_overlay_pairs.csv` 8 flags + a `bu1` provenance clause on every row's `source` (also on all 24 `rescreen_gap` rows); `bridge_classified_authoritative.csv` 6 flags + a `bu1` clause on every `note`, SHA-256 re-pinned (`00e4447a…8f6b769`, LF-normalised) in `build_all.py`, `PROVENANCE.md`, `REPRODUCING.md`; `water_separated_pairs.csv` base flags synced; `apply_overlays.py` byte-stable; `build_all.py` EXPECTED moved; a full `--full` rebuild from the pinned GeoPackages reproduces the shipped edge list and water table row for row (set-identical, 8,458 edges and 807 water rows; only the order of the appended overlay rows differs from the committed files, a pre-existing drift of the append history that is not part of this change); ADM0 roll-ups unchanged (26, 20/6).  Docs: METHODS `has_bridge` paragraph now states the four layers and their rules as one process for all rows; §8 campaign paragraph.  Record: `build_data/water_screen_rebuild/bridge_unification/`.

- **Domestic creek band adjudicated: 54 existing borders reclassified water-only (water-only 753 → 807; moderate 8,061 → 8,054; stringent 7,705 → 7,651; campaign dc1, 2026-09-14).**  The rebuilt HydroRIVERS screen's creek band (≥ 0.5 of a border within geodesic 500 m of any reach, < 0.5 at ≥ 10 m³/s) had been audited cross-border and wherever another rung co-nominated the edge; its 714 domestic creek-only edges had been set aside on 2026-07-10 as document-only. That gate is withdrawn (thresholds nominate, audits decide): the band went through the standard cross-vendor two-pass (GPT-5.6 Sol research → Sonnet-5 adversarial judgment, no repository access, 716 tool uses for 715 agents = the structured returns) together with the one edge restored on 2026-07-19 after the screen populations froze and therefore never screened (Balzers↔Planken, creek-band itself) — 715 edges; research water-only 65, judge 55; gate A 30 / B 60 / C 175 / D 450. Because the judges mostly derived their dry component from the complement of the HydroRIVERS share, a deterministic gap measurement (`measure_dc1.csv`: km of border > 500 m from any reach, longest contiguous run at 500/1,000 m, position on the arc) accompanied the 90 A/B pairs to the maintainer, whose map rulings (verbatim, `dc1_maintainer_rulings_2026-09-14.txt`) accepted 54 (20 dual-true, 21 research-only, 13 judge-only) and upheld 36 as land. Every accepted row passed the four-layer bridge pipeline (OSM Overpass screen with the in-both-units polygon test → agent verification → GPT-5.6 Sol adversarial recheck of the 35 disagreements/unknowns → coordinate cross-check against the shared WB arc): 47 bridged / 7 not. Rows appended to `rescreen_water_overlay_pairs.csv` (431 → 485), `verification_tier` A, `adjudication` cross-vendor (A 272 / B 238 / C 297); `apply_overlays.py` byte-stable; `build_all.py` EXPECTED and the manifest-count test moved; whole-graph attributability 807/807 (`attributability_check.py` now reads the campaign's screen record); the forward completeness check reports zero open nominations in every band. Record: `build_data/water_screen_rebuild/domestic_creek/`.

- **Corridor census presence-rule residue adjudicated; the post-hoc transect-support floor withdrawn (campaign nt3, 2026-09-11; docs only, data unchanged).**  The rebuilt corridor census's 80 new nominations split 8 by the 0.80 share rule, 30 only under the wide ladder and 42 by the short-gap presence rule; the nt2 extract had set aside the 33 presence-rule nominations below a 0.25 transect-support floor chosen after the fact. The floor is withdrawn (thresholds nominate, audits decide) and the 33 went through the standard cross-vendor two-pass (GPT-5.6 Sol research → Sonnet adversarial judgment, blind to priors → gate): all 33 rejected on convergent verdicts — 15 sea-separated harbours and straits (Grand Harbour, Marsamxett, the Bay of Kotor, Al Hoceima Bay, the Straits of Johor, the Kanmon Strait, the Caicos channel), 18 dry gaps with a third unit's land between (Maltese councils, Empty Quarter wadis, the Pešter plateau, Isle of Man parishes, Liechtenstein communes); gate A 0 / B 0 / C 3 / D 30, maintainer glance on C1–C3 with no override. Every corridor-census nomination is now adjudicated (79 of 80; the 80th is the already-ruled placeholder–Malawi point contact). METHODS §8 now also states the census's population and reach (with the lake-surface coverage check) and which thresholds are anchored versus inherited, with the bar sensitivity. Counts unchanged (edges 8,458; water 753 = 356/397; moderate 8,061; stringent 7,705; ADM0 326/320/300, roll-ups 26). Record: `build_data/water_screen_rebuild/corridor_census_v2/` (`nt3_*`).

- **Corridor census v2 and the nt2 recovery campaign: two non-touching water borders added (8,456 → 8,458 edges; water-only 751 → 753).**  The non-touching recovery census was re-derived (`build_data/water_screen_rebuild/corridor_census_v2/`): HydroLAKES (≥ 0.25 km²) joins NE lakes in the lake mask; lake polygons tested on a geodesic 125–1,500 m ladder; the HydroRIVERS test is a geodesic sample-to-reach distance (500 m; 1,000/2,500 m for reaches ≥ 1,000 m³/s) instead of the 2026-07 run's planar-degree buffer; the corridor is a set of transects across the facing frontage rather than one nearest-approach segment; third-unit and wedge flags recorded.  Diagnostic run over all 20,653 non-touching pairs within 100 km: all 22 shipped rescreen-gap rows re-nominated; 165 of the frozen census's 222 nominations re-found (the 57 lost are all adjudicated rejections); 46 new candidates (tier A share-rule 7 after excluding MOZXXX↔MWI003, tier B wide-ladder 30, tier C presence-rule with ≥ 0.25 transect support 9; 33 presence-only nominations below that floor dropped as noise).  Adjudication (nt2, 2026-09-10): GPT-5.6 Sol research → Sonnet-5 adversarial judge (blind to priors) → gate A 2 / B 4 / C 9 / D 31 → maintainer map rulings: **2 accepted** — Équateur↔Cuvette (COD/COG) across the Congo (dual-AI adjacent; only the 2,500 m big-reach rung nominates it) and Entre Ríos↔Artigas (ARG/URY) across Salto Grande (research adjacent on the Yacuy/Mocoretá limits; the judge's diagonal refutation rested on the third-unit statistic, which also fires on genuine shipped borders; maintainer: a short reach exists) — 44 rejected incl. Kandal↔Đồng Tháp (four-point corner touch), two Maltese harbour pairs (sea-separated) and Kaliro↔Serere (Pallisa separates).  Bridge pipeline: no structure in either corridor; both unbridged.  Counts: rescreen-gap 22 → **24** (6/18); edges 8,456 → **8,458** (cross-border 1,794 → 1,796); water 751 (356/395) → **753 (356/397)** = 639 river (344/295) + 114 lake (12/102); tiers A 216 → **218** / B 238 / C 297; attributability **753/753** with the v2 census recognised as the recorded corridor screen. Unchanged: moderate 8,061, stringent 7,705, regions 3,374, ADM0 326/320/300 and its 26 roll-ups (both country pairs carry land ADM1 crossings), rescreen-water 431, base 298, registry 3.  Method notes: the short-gap presence rule needs a transect-support floor when applied to transect sets; the 2026-07 census's river buffer was planar-degree, not geodesic as the docs said — corrected.

- **Pre-rebuild discovery nets retired as nomination steps (maintainer decision 2026-09-09; docs only, data unchanged).**  The ~1 km cross-border near-miss net and the 1–100 km lake band, the 5 km/10 km widening re-screens, the 2026-07-02/04 HydroRIVERS/HydroLAKES full-database cross-checks and, for the base classification, the original ~60%-within-2.5 km river screen are no longer presented as nomination mechanisms.  Proof: `build_data/water_screen_rebuild/hydro_fold/attributability_check.py` re-run on the shipped data — every one of the 751 water rows is nominated by the rebuilt screens alone (NE rungs ∪ HydroRIVERS/HydroLAKES domestic and cross-border rungs ∪ union-mask corridor census): base 298/298 by the NE rungs, widening 15/15, hydro-water 18/18, hydro-lakes 12/12, near-miss/lake-band rescreen-gap rows 6/6, domestic large-river 15/15, shore contacts 3/3.  The 51 folded rows keep their discovery provenance in `source`; the nets' audit records (the 48-pair near-miss funnel, the 10 km double-verification, the NE omissions that motivated the hydro rungs) remain validation evidence.  METHODS (overview, ledger rows, §8) and PROVENANCE reframed to one nomination story; two stale items fixed on the way — METHODS still said "4 of them retyped to river" (PR #108 missed this phrasing) and PROVENANCE's dataset table listed `rescreen_gap` as 16 rows (22 since 2026-07-23).  Counts, SHA pins and tiers unchanged.

- **Retype reversal: the four wu1 `lake → river` retypes (Itaipu, Kossou ×2, Volta) reversed by maintainer decision (2026-09-04); the per-arc "drowned channel" rule withdrawn.**  `water_type` returns to its original meaning — the Natural Earth layer that nominated the row — and the four rows carry their pre-ruling body names again (Represa Itaipu, Lac de Kossou, Lake Volta; Lagoa Mirim's body text likewise restored).  Descriptive only: `water_only`, `has_bridge`, tiers and every count are unchanged (edges 8,456; water 751 = 356/395; moderate 8,061; stringent 7,705; ADM0 326/320/300, roll-ups 26); the type split moves 641 river / 110 lake → **637 river / 114 lake**, the base classification 242/56 → **238 river / 60 lake** (298 rows).  Rationale: no package code reads `water_type`, and the retype forced a screened-vs-shipped reconciliation into every ledger table; the label now simply says which screen nominated the row.  Base bridge CSV SHA re-pinned (`build_all.py`, `PROVENANCE.md`); the dated 2026-07-25 entry below is left as written.

- **Rejection unification (rj1 + rj2) and Standard M: every verdict on record now carries the cross-vendor design; a never-queued domestic nomination band closed (+15 water-only rows).**  Maintainer decisions 2026-08-31 / 2026-09-01.  **rj1** re-adjudicated the 48 rejected candidates in the cross-border hydrography screen record whose only adjudication was a pre-rebuild design (two-stage ground truth / two-pass adversarial): GPT-5.6 Sol research (blind) → Sonnet-5 adversarial judge → deterministic measurement → maintainer gate; **all 48 upheld**, six by map ruling.  At that gate the maintainer ruled the water-only **acceptance standard is materiality (Standard M)**: a named surveyed line, ridge stretch, portage or overland connector defeats `water_only` regardless of its share of the arc, and the "~20%" figure in the adjudication prompts is an AI-pass screening tolerance only — the standard every prior maintainer ruling had already applied (`BRA025`↔`URY014`, not water-only at 10.2% dry) and one no shipped row contradicts (zero accepted verdicts quantify a 4–25% dry share); recorded in `docs/METHODS_adjacency.md` §8 (#105), `WATER_UNIFICATION_SPEC.md`, and the process docs.  **rj2** then swept the other screen records: 80 NE-rung frozen verdicts + 73 non-touching census rejections resting on the earlier designs (**all 153 upheld**; 2 closed mechanically below the census corridor bar) — so no verdict on record, shipped or rejected, rests on the pre-rebuild designs alone — **and 37 domestic large-river nominations (HydroRIVERS ≥ 0.5 in the ≥ 100 m³/s band) for which the 2026-07-10 queue registry had no domestic source — its medium-band source stopped at 100 m³/s, and large rivers were assumed covered by the Natural Earth rungs, which miss exactly these**, a genuine hole in the "every nomination adjudicated" claim.  Same pipeline (the judge required to name any dry component; 188 agents, zero repository reads); 32 dual-true items map-reviewed by the maintainer: **15 domestic river borders accepted** (Vjosa ×2, Nyong, Loange, Casanare/Meta, Lofa ×2, Saint John, Gauja, Tamesí–Pánuco, lower Imo, Mantaro, Katonga, Santa Lucía, Cầu, Đồng Nai), 17 declined — incl. three non-touching Yes rulings reversed on WB structure (Fălești↔Botoșani a Prut four-corner; Niassa↔Central/Northern Region mediated by the `MOZXXX`/`MWI004` lake-surface placeholders).  Crossings via the four-layer bridge pipeline (OSM Overpass screen, agent verification, adversarial recheck of 6, geocode + province-polygon cross-check — the Santa Lucía bridges fail the in-both-units test at 1.7 km from `URY016`): **11 bridged / 4 not**.  Rows appended to `rescreen_water_overlay_pairs.csv` (416 → **431**; tier A 193 / B 238 — the validation-study frame untouched).  Counts: water-only **736 (345/391) → 751 (356/395)** = 641 river (344/297) + 110 lake (12/98); moderate **8,065 → 8,061**; stringent **7,720 → 7,705**; tiers A 201 → **216** / B 238 / C 297; edges 8,456, ADM0 326/320/300 + 26 roll-ups, regions 3,374, base CSV 298, registry 3 unchanged.  Engine byte-stable no-op; `build_all` 12/12.  `docs/FUTURE_EDGE_AUDITS.md` gains #15 (Saramacca↔Sipaliwini: maintainer map review reads a point contact; WB arc stable 2.7 km; DECISION pending, no removal).  Campaign record: `build_data/water_screen_rebuild/rejection_unification/`.

- **Hydro-manifest consolidation: the cross-border extension of the rebuilt hydro rungs, then `hydro_water` + `hydro_lakes` folded into `rescreen_water` (registry 5 → 3; zero verdict change, zero count change).**  Maintainer decision 2026-07-28 (Option A: one whole-graph screen story).  **Step 1 — the extension (evidence, no data change):** the rebuild's HydroRIVERS/HydroLAKES rungs had run over *domestic* borders only, cross-border hydro coverage being inherited from the 2026-07-02/06 full-database sweeps; the same instrument (shared-arc sampling, geodesic 500 m) was run over all **1,794** cross-border edges → `crossborder_hydro_disposition.csv` (358 already-shipped / 292 frozen-verdict / 1,122 not-nominated / 20 non-touching / **2 NEW-candidates** gated to the maintainer, not shipped).  **All 14 previously-unrecorded shipped rows re-nominate** (river 0.776–1.000, lake 0.709 — and the seven ru1 demotions re-measured within ~0.003 of ru1's instrument, an independent cross-validation), and **whole-graph attributability holds: 736/736** shipped water rows are attributable to a recorded deterministic screen (NE rungs ∪ hydro domestic ∪ hydro cross-border ∪ recovery census).  **Step 2 — the fold:** the 18 `hydro_water` + 12 `hydro_lakes` rows moved into `rescreen_water` (386 → **416**) with their two-stage-ground-truth + 100%-human-map-verification provenance retained additively in `source` (`water_type` from the retiring registry constants; `border_km` dropped — the arc lives in the edge list); both manifests deleted; registry **5 → 3** (land_gap, rescreen_gap, rescreen_water).  The 30 water-CSV `note` values were canonicalized to the rescreen-water registry note after a value-neutral proof (all 30 agreed with their manifest on `has_bridge`/`water_type`/`water_body`) — the lg1 note-match trap handled head-on, not tripped.  Counts **unchanged**: edges 8,456; water 736 (345/391); moderate 8,065; stringent 7,720; ADM0 326/320/300, 26 roll-ups; base CSV 298 (SHA pin untouched); tier B 238; `adjudication` uniformly cross-vendor.  Engine byte-stable no-op immediately after the fold; `build_all` 12/12.  The hydro sweeps' discovery history stays in PROVENANCE as frozen validation record — the fold moves rows, not history.  Campaign record: `build_data/water_screen_rebuild/hydro_fold/`.

- **Water-screen unification (wu1): the last 94 water-only rows re-adjudicated under the standard cross-vendor process — one adjudication process now covers all 736 shipped rows; 3 mid-lake point/corner contacts removed by maintainer map ruling; 4 dam impoundments retyped lake → river.**  ru1 (2026-07-21) unified the river class; **94 rows had still never been through the cross-vendor two-pass**: the **63 base lake rows** (classified on the Natural Earth lakes layer alone — an instrument measured **below its own 0.40 bar on 97 of 139 HydroLAKES-positive queue rows, 70%**; 57 of the 63 carried `note=huge-lake/knowledge`, i.e. no screen ever nominated them), the **18 `hydro_water`** and **12 `hydro_lakes`** rows (two-stage ground truth + human map verification), and the Skadar `rescreen_gap` row.  All 94 were re-run blind — 92 through the research pass, 2 placeholder↔placeholder pairs routed straight to the maintainer.  **The measurement instrument had to change**: ru1's `frac_500 >= 0.80 ⇒ uphold` rule is **non-discriminating for lakes** (a genuine mid-lake border and a corner artifact both score 1.00), so lake-class contests were measured on the **arc-tolerance ladder** (arc length + component count at 1e-3 / 5e-3 / 2e-2) and treated as **advisory**, with the maintainer's map ruling deciding; the `>= 0.80` rule stayed decisive only for the 18 river-class `hydro_water` rows.  **Maintainer rulings.** *Not adjacent → removed* (3): Kasba Lake `CAN003`↔`CAN006` (four-corners point), Lake Edward `COD009`↔`UGA102` (point contact), Lake Victoria `TZA016`↔`UGA040` (diagonal non-adjacency; the neighbouring `TZA016`↔`UGA088` was upheld as genuine).  **These are the project's first removals against a *stable* WB arc** — 0.563 / 1.081 / 14.019 km at 1e-3, one component each, no collapse signature — so unlike the 2026-07-18 and 2026-07-22 precedents they rest solely on the maintainer's official-map check, which the evidence hierarchy permits as its top tier; recorded as such in `docs/FUTURE_EDGE_AUDITS.md` #11–#13.  Because these are **native S1 edges**, removal went into `_ADM1_FALSE_POSITIVE_DENYLIST` (2 → **5** pairs), not a manifest — an edge-list-only edit would have been undone by the next `--full` rebuild.  *Disputed areas → upheld with no political stance* (5): Bodensee ×2, Lake Nyasa ×2 and the Dead Sea were settled by the **WB+NE test** — WB holds a stable shared arc **and** NE/HydroLAKES say that arc is water, so water-only stands; the "undefined line" and sovereignty objections are legal questions neither dataset encodes.  *Type refinement* (5): the 2-value `water_type` is kept, and Itaipu, Kossou ×2 and Volta are **retyped `lake` → `river`** as dam impoundments of border rivers (reservoir named in `water_body`); Lagoa Mirim stays `lake`.  *Upheld* : the 8 river-class challenges the ladder measured at 0.887–0.985, and the 2 placeholder pairs.  **A rubric defect was found and corrected mid-campaign**: the research prompt inherited ru1's river wording ("the whole border follows ONE water feature"), and the research pass read that lead-in as a necessary condition, returning `water_only=false` for six borders it simultaneously described as entirely water (explicitly, for Ontario↔Michigan: "the one-feature test fails despite an all-water international alignment").  The judgment pass was given the corrected definition and overturned all six — the adversarial pass doing exactly what it exists for — and the generator is fixed at source.  **ru1 was re-checked, not assumed**: all 9 of its demotions measured 0.407–0.770, i.e. below the 0.80 bar, so each had independent deterministic support, and its auto-uphold rule is precisely the safeguard against the same ambiguity; ru1 stands, no re-run.  Counts: edges 8,459 → **8,456**; water-only 739 (345/394) → **736 (345/391)** = 626 river (333/293) + 110 lake (12/98); base classification 301 → **298** rows (242 river / 56 lake; SHA re-pinned in `build_all.py`, `PROVENANCE.md`, `REPRODUCING.md`).  **Unchanged**: moderate 8,065, stringent 7,720, regions 3,374, ADM0 326/320/300 and its 26 roll-ups, overlay registry 5.  `hydro_water` (18), `hydro_lakes` (12) and the Skadar row gained wu1 provenance **additively**, retaining their existing human-map-verification record rather than replacing it.  One item was routed out of scope rather than acted on: `BOL004`↔`PER007`, where WB's polygon named *Callao* (Lima's Pacific port) is a 4-part shape centred in the Puno altiplano whose contact with La Paz falls inside Lake Titicaca — an edge-existence/unit-identity question, registered as `docs/FUTURE_EDGE_AUDITS.md` #14 and shipped unchanged.  Engine byte-stable no-op; `build_all` 12/12.  Campaign record: `build_data/water_screen_rebuild/water_unification/`.

- **Lake-gap retirement (lg1): the last lake-gap row (Skadar) folded into rescreen-gap; the lake-gap overlay retired.**  The symmetric bookend to rg1. After the 2026-07-22 Peipus removal the lake-gap overlay carried a single row — Malësi e Madhe↔Bar across Lake Skadar (ALB022↔MNE002).  It was folded into `rescreen_gap_overlay_pairs.csv` (21 → **22** rows; now 19 river / 3 lake, 6 bridged / 16 not) with lg1 provenance; `lake_gap_overlay_pairs.csv` **deleted**; overlay registry **6 → 5**.  **No re-adjudication** (unlike rg1): Skadar's verdict is already top-tier — the two-pass adversarial adjudication of all 57 water-band candidates (1 genuine / 56 rejected) plus human map verification (2026-07-04) — and the rebuild's recovery census independently re-nominates it (frac_lake 0.98), so the fold is pure manifest consolidation, verdict unchanged.  The 1–100 km water-corridor band census is now a historical instrument with zero shipped manifests of its own (Peipus removed, Skadar folded); its 57-candidate record stays frozen as validation evidence.  Counts **unchanged**: edges 8,459; water-only 739 (345/394); moderate 8,065; stringent 7,720; ADM0 326/320/300, 26 roll-ups; regions 3,374.  Engine byte-stable no-op; `build_all` 12/12.  Campaign record: `build_data/water_screen_rebuild/lake_gap_fold/`.

- **Peipus removal + river-gap retirement (rg1): the old ~1 km near-miss net retired as a provenance vintage; two corner-contact artifacts removed by maintainer map ruling; the river-gap overlay folded into rescreen-gap.**  (1) **Jõgeva↔Pskov (EST006↔RUS061, Lake Peipus) removed 2026-07-22**: the original verdict's map source (Google) was withdrawn by the maintainer; the WB-internal signature shows a Tartu–Jõgeva–Pskov **corner contact** (nearest gap 0.0 m at a point shared with Tartu; taper profile, no shared segment) — `lake_gap` manifest 2 → **1** row (Skadar only).  (2) **rg1 re-adjudication of the river-gap overlay's 6 pairs** under the standard cross-vendor process (GPT-5.6 Sol web research, blind → Sonnet-5 adversarial judgment, 6/6): four dual-AI confirms (Tulcea↔Odesa, Caraș-Severin↔Borski, Saarland↔Grevenmacher, French Guiana↔Marowijne); **Galați↔Odesa KEPT by maintainer map ruling** — adjacent via **Insula Cailor**, ~0.5 km water frontage, arc corrected 0.02 → **0.5 km** — overruling a dual-AI not-adjacent verdict and the WB corner signature (706 m min gap at the Giurgiulești tripoint, 39 m from Cahul); **Cahul↔Vaslui (MDA006↔ROU041) DEMOTED by maintainer map ruling** — a Cahul/Cantemir–Vaslui/Galați **four-corner artifact** (judgment caught the research pass citing the Cahul–Oancea bridge, which is in Galați county; WB signature: contact point ON Galați, every Cahul boundary point near Romania closer to Galați than Vaslui, 677:0; the true frontages Cahul↔Galați 113 km and Cantemir↔Vaslui 74 km were always native edges).  (3) **Fold**: the 5 surviving rows moved into `rescreen_gap_overlay_pairs.csv` with rg1 provenance (16 → **21** rows); `river_gap_overlay_pairs.csv` DELETED; overlay registry 7 → **6** (the dead river_gap edge-style removed from the engine); the ~1 km near-miss net is retired — its 48-census record (incl. the 19 audited dry negatives) stays as frozen validation evidence, and the maintained cross-border census gains the two re-entered members (pre-adjudicated by these rulings).  Counts: edges 8,461 → **8,459**; water-only 741 → **739** (345/394); moderate **8,065** and stringent **7,720** unchanged (both removals lenient-only); ADM0 326/320/300, roll-ups 26 (20/6), regions 3,374 unchanged.  Engine byte-stable; `build_all` 12/12.  Campaign record (spec, frozen queue, research + judgment verdicts, WB signatures, rulings): `build_data/water_screen_rebuild/river_gap_fold/`.

- **River-screen unification (ru1): every pre-rebuild river-class water verdict re-adjudicated under the standard cross-vendor process; the wide-river and audit-water overlays folded into the rescreen manifest; 9 verdicts demoted to mixed land by maintainer map ruling.**  All 262 pre-rebuild river verdicts (the base classification's 246 river rows + the 13 wide-river + 3 audit-water overlay flags) were re-adjudicated **blind** under the standard process (GPT-5.6 Sol web research — validator content checks passed — → Sonnet-5 adversarial judgment, 262/262), the 38 contested verdicts measured deterministically (the IRN013 instrument: share of the exact WB border within geodesic 500 m / 2 km of HydroRIVERS/HydroLAKES), and the 16 pairs the measurement could not settle map-reviewed by the maintainer (2026-07-21).  Outcome: 216 clean dual-AI confirmations; 37 upholds (22 decisive at ≥0.80 water-share — incl. **Vratca↔Olt at 1.00**, vindicating its 2026-07-02 human adjudication against a judge-side challenge — 8 flagged agreements, 7 maintainer rulings); and **9 demotions to mixed land** (LAO002↔THA010 Mekong, HRV006↔HUN015 Drava, ARG009↔PRY016 Pilcomayo/Estero Patiño, CHE026↔DEU001 Rhine/Rafzerfeld, ARM007↔TUR045 Akhurian, LSO004↔ZAF002 Caledon, AFG001↔TJK001 Panj/Pamir, AGO005↔NAM005 Okavango, CMR003↔COG012 Dja–Sangha) — ordinary land edges now, pericoupled under every standard.  The **wide-river and audit-water overlays are retired**: their 15 surviving rows folded into `rescreen_water_overlay_pairs.csv` with ru1 provenance (371 → **386** rows), the overlay registry 9 → **7**; the base bridge CSV drops to **301** rows (SHA-256 re-pinned).  The NE river screen (2.5–20 km ladder) is now one uniformly adjudicated step.  Counts: water-only **750 → 741** (345/396); ADM1 moderate **8,063 → 8,065**, stringent **7,711 → 7,720**; ADM0 roll-ups **28 → 26** (the bridged AFG↔TJK and ARG↔PRY roll-ups dissolve), ADM0 stringent **298 → 300**; edges (8,461), regions (3,374), ADM0 326/320 unchanged.  Campaign record (spec, frozen queue, research + judgment verdicts, measurement, rulings): `build_data/water_screen_rebuild/river_unification/`.

- **Future-edge-audit register executed: two artifact edges removed, one GAUL-split unit merged, all placeholder units identified (`docs/FUTURE_EDGE_AUDITS.md`).**  All 10 queued items resolved (deterministic geometry passes + five research adjudications, 2026-07-18).  Maintainer decisions: **Grand Gedeh↔Rivercess removed** (a river-confluence quadripoint stretched into a 1.12 km cardinal-leg connector; OSM and GADM both show a point contact) and **Apure↔Amazonas removed** (Amazonas' territorial-division law names no Apure neighbour; the 2.35 km contact is a mid-river seam where Bolívar's east-bank sliver was dropped) — both added to `_ADM1_FALSE_POSITIVE_DENYLIST`; **RUS050 ("Name Unknown") merged into RUS024 Kalmykia** via the new reviewed `_ADM1_UNIT_MERGES` build step (the polygon is the republic's own Gorodovikovsky + Yashaltinsky salient, split and left unnamed upstream in FAO GAUL; its Rostov/Stavropol frontages fold into the existing Kalmykia rows); **MOZXXX↔MWI003 removed by reversing its land-gap recovery** (maintainer map ruling: the true contact is a point — the WB arc collapses to one coordinate under tight tolerance; `land_gap_overlay_pairs.csv` 5 → 4); and **three shore contacts of proven water-surface polygons maintainer-confirmed water-only** (KHM000↔KHM004, KHM000↔KHM006 — Tonlé Sap; ZWE011↔ZWE008 — Lake Kariba; lake, no fixed crossing; rescreen-water manifest 368 → 371).  Retained/validated: Osijek-Baranja↔Bács-Kiskun confirmed a genuine ~8 km border (dispute-contingent on the Croatia–Serbia tripoint-pocket attribution); the Seymareh water flag upheld by measurement (88.4% river at 500 m); KHM000 proven the Tonlé Sap water-surface polygon, ZWE011 Lake Kariba's, OMN009 Oman's Madha exclave, LCA001 Saint Lucia's crown-land tract.  Count effects: ADM1 edges **8,467 → 8,461**, regions **3,375 → 3,374**, water-only **747 → 750** (352/398), moderate **8,072 → 8,063**, stringent **7,720 → 7,711**; the ADM0 layer unchanged.

- **Batch b2 adjudication completed under the standard cross-vendor two-pass design (+49 water-only flags; b2 total 125; all six tranches uniform).**  The remaining b2 verdicts were adjudicated by the standard process — GPT-5.6 Sol web research over all 393 pairs (handoff validator: 0 content problems) → Sonnet 5 adversarial judgment — with a 69-row human map triage: **49 water-only flags shipped**, 40 maintainer-accepted and 9 resolved by a human-delegated **Fable 5 final arbitration** — seven of the eight pre-2021 Latvia/Lithuania vintage pairs arbitrated (adjudicated on the shipped WB geometry itself; the eighth, Telšiai↔Saldus, rejected) and the contacts of the two `Area under National Administration` polygons, identified from their WB adjacency signatures as the Malawian and Mozambican **Lake Malawi/Niassa water-surface units** (1954 median-line boundary, IBS No. 112); one arbiter-internal disagreement was settled by a deterministic re-measurement tie-break (frozen with the verdicts).  Every accepted pair passed the four-layer bridge pipeline (49 pairs: 32 bridged / 17 not; the province-polygon layer corrected one off-border citation — the Kerio crossing — to the on-reach Kolowa Rd–Rorok Rd bridge, WB polygon test PASS).  Count effects: water-only **698 → 747** (352/395), ADM1 moderate **8,089 → 8,072**, stringent **7,769 → 7,720**; ADM0 roll-ups **27 → 28** — MOZ↔TZA completes as all-water (bridged, so ADM0 moderate stays **320**; stringent 299 → **298**).  All frozen verdicts under `build_data/water_screen_rebuild/verdicts/`.

- **Preregistered validation study of the water-only classification (uncertainty quantification).**  Sampling plan and deterministic sampler committed before the draw (`docs/VALIDATION_SAMPLING_PLAN.md`, seed 20260713); 180 rows rated by the maintainer plus a 30-row blind second-rater subsample.  Results (`docs/VALIDATION_STUDY.md`): dual-AI-tier precision **98.7%** (1/76; exact 95% CI on the error rate [0.03%, 7.1%]), weighted false-omission among AI-adjudicated negatives **≈ 1.1%** (1/39 in the high-signal stratum, 0/39 rest), mechanism auto-reject **0/20**, inter-rater **κ = 0.867**.  Both discovered errors corrected via the manifest path: `NIC016↔NIC017` (Río San Juan↔Rivas) **demoted** — the departments share a land border south of Lago Cocibolca — and `ITA005↔ITA020` (Emilia-Romagna↔Veneto, the Po) **added** with the four-layer bridge protocol (SS16 Pontelagoscuro + A13 Occhiobello ⇒ bridged).  Net counts: water-only unchanged at **698**, bridge split 319/379 → **320/378**, ADM1 moderate 8,088 → **8,089**.  PROVENANCE now defines the three **verification tiers** (A human-verified / B dual-AI convergent / C reviewed base) with these measured error bounds, and frames the dataset as a **versioned, uncertainty-aware adjacency benchmark** with a documented correction path.  Side records: `MLT009↔MLT055` rater-flagged non-adjacent (future-edge-audit list); Latvia 2021 subdivision-vintage caveat documented.

### Fixed

- **Four notes of the source-relabel manifest quoted, so every row parses to its seven fields (no value, edge or count changes).**  Four rows of `data/sliver_corridor_relabel.csv`, those of the hosts Gedo, Wajir, Galgaduud and Lamwo, carried a comma inside an unquoted `note`, so Python's `csv` read eight fields against the seven-column header: `csv.DictReader` returned each note cut short after "km" and filed the rest (" verified by live re-run 2026-09-22).") under the `None` key.  The comma entered with the notes' 2026-07-07 wording (#82) and survived their 2026-09-22 update (#131).  The four notes are now quoted, two quote characters each, and every other byte of the file is unchanged.  The build reads only the host, owner and opening columns, and they parse identically before and after (`load_relabel_manifest()`: the same ten hosts, eleven owners and 2×10⁻³° radius, in the same order; the build's relabeled ADM1 polygons hash identically), so the relabel and every edge, length and count are unchanged.  Tests +1: `tests/test_data_csvs.py` parses every CSV under `src/metacouplingllm/data/` in strict mode and requires each row to carry exactly its header's field count; on the file as it was it fails, naming lines 6–9.

- **The source-relabel leaves no zero-width spike, and each owner takes exactly the frontage its host gives up (no edge added or removed; every headline count unchanged; 29 border lengths change).**  The relabel detaches each reviewed sliver corridor by a morphological opening, whose outline meets the host's raw outline at cut points inside raw segments, about 10⁻¹⁶° off the line.  The unit across the border had no vertex there, so the host kept the rest of each such segment as a zero-width spike, which the exact-contact measure counted as its frontage, and the owner's new frontage matched no neighbour's line, so the measure missed it: Lamwo↔Eastern Equatoria read 158.99 km, 8.36 km of it Kitgum's; Kitgum↔Eastern Equatoria 12.92 km for 21.28; Garissa↔Wajir 265.10 km for 229.70 (a 17.7 km spike and slit counted twice); and 9.4 km of the Kenya–Somalia border sat in no edge at all.  Each cut point is now a vertex of every outline through it, so the host, the owner and the unit across the border share the new junction.  A corridor whose cut falls on a raw segment that faces a third unit and ends at a junction closes at that junction (maintainer ruling, 2026-09-22): Salta's corridor, whose cut falls 304.2 m short of the Potosí–Tarija point, and Braničevo's, 13.8 m past the Caraș-Severin–Mehedinți point.  Without the closing the exact measure would have brought back a Salta↔Potosí edge and turned the reviewed non-touching Bor↔Caraș-Severin Danube border into a native contact.  Salta's opening radius returns to the default 2×10⁻³° (maintainer ruling): the 8.2 km of residual Salta↔Potosí contact that had justified 3×10⁻³ was such a spike.  Every group of touching relabels now conserves each neighbour's frontage exactly, the global area drift is below 0.0001 km², and no spike remains.  Border lengths that change include Kitgum↔Eastern Equatoria 12.92 → 21.28 km, Lamwo↔Eastern Equatoria 158.99 → 150.63, Garissa↔Wajir 265.10 → 229.70, Galgaduud↔Mudug 517.58 → 487.85, Lamwo↔Kitgum 173.67 → 153.63, Wajir↔Lower Juba 72.46 → 85.09, Garissa↔Lower Juba 250.37 → 255.59, Mudug↔Ethiopian Somali 44.53 → 46.23, Kajiado↔Kilimanjaro 44.67 → 47.94 and Narok↔Mara 62.58 → 67.47; no `narrow_border` or `potential_artifact` flag changes.  The tolerance sweep of METHODS §2.4 was re-measured with a re-runnable script (`build_data/snap_sweep/snap_sweep_tol.py`, which reproduces the 2026-07-07 record exactly on the relabel before this fix): 8,425 / 8,428 / 8,428 / 8,431 / 8,436 / 8,451 / 8,515.  Braničevo↔Mehedinți, the fabricated edge the relabel removes, now meets at a single point, so the 55 m band holds six pairs; the ≤ 1 km near-miss census keeps its members.  The border-arc record was regenerated in full on the fixed polygons: 31 edges' arcs change, every one an edge of a unit the fix touched; the exact line loses 90 km of spike (1,227,468 → 1,227,377 km), the overlap class shrinks from 11 edges to the four Saint-Louis borders (456 → 444 km), and no screen's nomination changes on any edge.  Tests +3: no spike in any host or owner; Kitgum's frontage measured in full with the two units' total conserved; the two junction closings.  Record: `build_data/sliver_remnant/` (rulings, prototypes, maps, `check_relabel_fix.py`, `relabel_length_delta.txt`).

- **External-review corrections (data + docs).**  An independent model review of the shipped database surfaced five documentation/schema defects, all confirmed and fixed: (1) `build_data/bridge_classified_authoritative.csv` — a required `--full`-rebuild input — is now **tracked in the repository** (SHA-256 verified against the pin before adding), closing the gap where a clean clone could not run the full rebuild; `docs/REPRODUCING.md` now states exactly what a clean clone contains.  (2) The blanket "human map verification of every shipped overlay verdict" claim in PROVENANCE is scoped honestly: it holds for the six pre-rebuild overlays, while the rescreen overlays carry human verification of every ship-affecting disagreement/medium-confidence verdict (~157 rows) and 240/335 high-confidence dual-AI auto-accepts labelled per row — convergent model evidence, not independent ground truth.  (3) "Completeness is fixed by the sweeps" reworded everywhere to **candidate-screen coverage** relative to the datasets' documented floors — not a recall claim against the unknown true adjacency set.  (4) Count drift: METHODS "removed 3" → 4 fabricated edges, "Eleven" → 12 HydroLAKES reclassifications; PROVENANCE note-block counts and one 696 → current (8,467 / 8,088 / 7,769; water-only 698).  (5) `land_gap_overlay_pairs.csv`: the `gap_km` column duplicated `border_km` (Egypt–Libya read "49.38 km" where the documented digitization gap is ~0.4 m) — column dropped (nothing reads it; the true gaps are documented in METHODS §2.4/§4), and a stale label was removed from its `source` strings.  Engine remains a byte-stable no-op; all headline counts unchanged.

### Added

- **Water-screen rebuild, 20 km-hold tranche: 2 final water-only flags — the 2,158-pair audit queue closed.**  The 211 pairs first captured only at the 20 km river rung had been held conditional on the 15 km gate; the gate proved **not dry** (8 judge-true among 247 judged 15 km-first-capture pairs — mostly lake borders and NE-absent small rivers, i.e. Natural Earth *selection* effects, not 15 km-wide rivers), so per the standing rule the holds ran as the final tranche (GPT-5.6 Sol research with web, content checks passed → Sonnet judgment: 3 research-true → 3 judge-true).  Outcome: **Brăila↔Tulcea** across the Danube's Măcin branch (dual-AI high-confidence; the July-2023 **Brăila Bridge** — "the first road connection between Tulcea and Brăila counties" — validated by a WB-boundary segment-chain test after the OSM screen missed the 2 km multi-segment suspension structure) and **Central↔Ñeembucú** across the Arroyo Paray (human-accepted 2026-07-13; PY19 "Héroes de 1870" bridge, whose boundary river is a member of both departments' OSM admin relations) ship as bridged flags; Kayunga↔Luwero (Lwajjali, low confidence) human-rejected.  Counts: water-only 696 → **698** (319/379); ADM1 stringent 7,771 → **7,769** (moderate **8,088** unchanged — both new rows bridged); ADM0 (326/320/299) and roll-ups (27) unchanged.  Strictly additive, byte-stable.  With this, every candidate of the rebuild's 2,158-pair Tier-2 audit queue is disposed: 335 shipped / 288 mechanism-rejected / the rest adjudicated not-water-only.
- **Water-screen rebuild, batches b3–b6: 169 further water-only flags — closing the audit queue's main tranche.**  The combined final batches (837 pairs after the mechanism-ledger exclusions) ran the standard cross-vendor two-pass (GPT-5.6 Sol research with verified web access — the new validator content checks passed: 677/837 high-confidence, real citations — → Sonnet 5 adversarial judgment: 239 research-true → **179** judge-true, 62 flips).  Human review of the 35 disagreement/medium rows (2026-07-12): 25 accept / 7 reject / **3 not-adjacent verdicts recorded to the future-edge-audit list** (Osijek-Baranja↔Bács-Kiskun — consistent with its 2026-07-02 water-set demotion — plus one Liberian and one Venezuelan pair; edges retained per the WB-internal-signature rule); +144 dual-AI high-confidence auto-accepts = **169 shipped flags** (all existing edges, zero new).  The four-layer bridge pipeline classified them **103 bridged / 66 not** after 28 OSM-vs-agent disagreements were adversarially rechecked (20 OSM wrong / 7 agent wrong / 1 both) and the geocode + province-polygon layer resolved 14 pin failures: 6 validated as mid-span pins in the WB inter-polygon water strip (<0.75 km from both units — a documented rule for wide-river crossings), 6 corrected to the true on-border structures (incl. the Dunaföldvár Beszédes József híd replacing Baja's Türr István híd, both of whose ends land in Bács-Kiskun via the Danube side-channel anomaly, and the Paklay Mekong bridge in Laos), and 2 flipped to no-bridge (Buliisa↔Nwoya — the Paraa Bridge belongs to Masindi↔Nwoya, separately and correctly recorded; Sabirabad↔Shirvan — the true shared reach is a 1.2 km unbridged Kura stretch).  Counts: water-only 527 → **696** (317/379); ADM1 moderate 8,154 → **8,088**, stringent 7,940 → **7,771**; ADM0 roll-ups 26 → **27** — **MWI↔TZA** joins (the bridged Songwe plus the Lake Malawi/Nyasa median segments), staying in moderate: ADM0 **326**/**320**/**299**; edges (8,467) unchanged.  Strictly additive (339 insertions / 0 deletions), byte-stable, verified vs the frozen baseline.
- **Water-screen rebuild, batch b2: 76 further water-only flags — including the first moderate-view ADM0 change (GUY↔SUR).**  Batch b2 (393 pairs) ran the same Tier-2 protocol (GPT-5.6 Sol research → Sonnet 5 adversarial judgment; the handoff validator enforces **content checks** — template-dominance and a source-citation floor — on every research handoff).  Human review of the 57 disagreement/medium rows (2026-07-12): 43 accept / 13 reject / 1 as-is, incl. **5 human overrides of AI refutations**; +33 dual-AI high-confidence auto-accepts = **76 shipped flags** (all existing edges, 57 domestic / 19 cross-border — zero new edges).  The four-layer bridge pipeline classified them 55 bridged / 21 not; its deterministic geocode + **province-polygon test caught three off-border citations** — the Cuanza EN120 crossing flipped to no-bridge (both bridge sites lie wholly off the Cuanza Norte↔Cuanza Sul shared reach; no crossing exists on the ~50 km river-following boundary), and two crossings corrected to the true on-border structures (Nova vas ob Sotli–Draše over the Sotla — the original citation traced to Slovenia's 2014 statistical-region boundary change — and the Gulu–Kitgum Road Aswa bridge).  Counts: water-only 451 → **527** (214/313); ADM1 moderate 8,175 → **8,154**, stringent 8,016 → **7,940**; ADM0 roll-ups 23 → **26** — **BEN↔NER** (Niger/Mékrou, Malanville bridge) and **CMR↔GAB** (Ntem, bridged) join and stay in moderate, **GUY↔SUR** (Corentyne, ferry-only) joins and drops from moderate, the rebuild's first moderate-view ADM0 change: ADM0 **326**/**320**/**300**; edges (8,467) unchanged.  Strictly additive, byte-stable, verified vs the frozen baseline.
- **Water-screen rebuild, batch b1: 88 water-only borders added (72 flags + 16 recovered edges) via the rescreen overlays — strictly additive.**  Every water-screen threshold was re-anchored (NMAS/NSSDA map-accuracy anchors, measured capture ladders) and the screens re-run over the full edge list with **domestic borders in scope for the first time** and lowered, measured bars: NE river 0.50 at geodesic 2.5/5/10/15/20 km rungs, NE lake 0.40 at geodesic 125/250/500/1,000/1,500 m, HydroRIVERS nomination floor 100 → **10 m³/s** plus the full creek band (which alone yielded 13 genuine borders — discharge is a poor proxy for border-worthiness), HydroLAKES bar 0.6 → **0.5**, and a union-mask non-touching recovery rule (corridor ≥ 0.80 inside NE lakes ∪ geodesic-500 m HydroRIVERS buffer).  Every candidate was **Tier-2 adjudicated** with a cross-vendor two-pass design — research on OpenAI's GPT-5.6 Sol (in the Codex app), adversarial judgment on Claude Sonnet 5 — then gated: medium-confidence verdicts human map-verified, high-confidence dual-AI cross-checked, and proposed new edges screened against the frozen diagonal-lake precedent (18 long-gap diagonals rejected with the precedent cited; 15 sibling rejections by human review).  Batch b1 (420 pairs) ships two new reviewed manifests applied by `scripts/apply_overlays.py`: `data/rescreen_water_overlay_pairs.csv` (**72** existing edges — e.g. the Our/Sauer, Suam, Astarachay — reclassified water-only; flags only) and `data/rescreen_gap_overlay_pairs.csv` (**16** non-touching water borders restored as edges — e.g. the Gambia River, the Elbe Brandenburg↔Niedersachsen, Lac de Biaufond, the Vietnamese Mekong distributaries; `border_km` = geodesic facing-arc length), with per-row `water_type` and full verification provenance in `source`.  Bridge flags for all 88 come from the four-layer bridge pipeline (OSM Overpass screen → agent web verification → adversarial recheck of every disagreement → geocode + province-polygon cross-check): 34 bridged / 54 not.  The change is **verified strictly additive** against a frozen SHA-256 baseline of the pre-rebuild data (no row removed or altered).  Counts: shipped ADM1 edges 8,451 → **8,467** (moderate 8,213 → **8,175**, stringent 8,088 → **8,016**); water-only 363 → **451** (159 with a fixed crossing / 292 without); ADM0 roll-ups 22 → **23** — **DEU↔LUX** joins (the Our/Sauer flag completes the Germany–Luxembourg border as the fully bridged Our–Sauer–Moselle chain), so ADM0 stringent 304 → **303** (326 lenient / 321 moderate unchanged); regions **3,375** and countries **196** unchanged.  Remaining audit batches (b2–b6) extend the same two manifests additively.  Method + evidence: `docs/METHODS_adjacency.md` (ledger + §8), `data/PROVENANCE.md`, `build_data/water_screen_rebuild/`.
- **Geodesic water-buffer re-screen: two water-only borders the degree screens missed (Uruguay River, Dead Sea).**  The three water proximity screens (the 2.5 km river-centerline screen; the ~500 m HydroRIVERS and ~300 m HydroLAKES buffers) had used degree approximations, which under-measure east–west distance at latitude (a raw-degree buffer reaches only `X·cos φ` metres E–W) — the one direction a completeness screen must *not* err.  Re-running all three in true geodesic metres (`GEOD.inv`; a single uniform **500 m** buffer for both HydroRIVERS and HydroLAKES; cos-φ-widened candidate query) reproduces the frozen flag set **bit-for-bit** under a planar control (proving only the metric changed) and surfaces two genuine water-only borders the degree screens had missed: the **Uruguay River** (`ARG008`↔`URY012`, Entre Ríos↔Río Negro — HydroRIVERS coverage 0.46, just under the 0.5 flag bar, vs geodesic 0.54; the Libertador San Martín bridge makes it moderate-pericoupled) and the **Dead Sea** (`ISR005`↔`JOR007`, Southern District↔Karak — HydroLAKES 0.51 at 300 m vs 0.80 at 500 m; no crossing → lenient-only, so it leaves the moderate view).  Both were two-pass adjudicated water-only (1961 Uruguay River Treaty; 1994 Israel–Jordan Treaty Annex I); three further candidates the wider net raised (the Burundi–Rwanda inter-lake connector and two Norway–Sweden segments) were adjudicated mixed and rejected — *thresholds nominate, audits decide*.  Originally shipped as `data/hydro_geodesic_river_overlay_pairs.csv` + `data/hydro_geodesic_lake_overlay_pairs.csv`; both files were later folded into `hydro_water_overlay_pairs.csv` / `hydro_lakes_overlay_pairs.csv` (PRs #82/#84) and deleted (flags only, no new edges).  Counts: water-only **361 → 363** (124/237 → 125/238); ADM1 moderate **8,214 → 8,213**, stringent **8,090 → 8,088**; shipped edges **8,451**, ADM0 (**326**/321/304), and regions **3,375** unchanged.  Method + adjudication: `build_data/geodesic_rescreen/` (`REPORT.md`, `adjudication.json`); `docs/METHODS_adjacency.md` §2.5 units policy and `data/PROVENANCE.md` updated.  No edge-count change.
- **Water-only river screen documented at 2.5 km (from 2.4 km); shipped set proven invariant.**  The river candidate screen flags a cross-border pair when >60% of its shared border lies within *D* km of a major named Natural Earth river centerline.  The documented threshold is standardised to the rounder **2.5 km** (from 2.4 km).  A re-screen over all 1,800 cross-border edges (validated per-sample-point distance cache, correlation 0.84 vs the original `ne_cov`) shows the change is **membership-neutral**: `screen(2.4 km)` = 283 candidates, `screen(2.5 km)` = 284, and the single new entrant — `BWA009`↔`ZAF007` (the Notwane/Ngotwane border, South-East Botswana ↔ North West South Africa) — was already ground-truthed three times in the 5 km/10 km completeness audits as **mixed river-and-land, not water-only** (US State Dept International Boundary Study No. 122, 1972).  It is correctly absent from the shipped set, so the water-only totals (**361** ADM1 = 124/237; **22** ADM0 roll-ups) and every edge count are unchanged — consistent with *thresholds nominate, audits decide*.  Reproducible via `build_data/wide_river_audit/screen_threshold_2p5km.py`; `docs/BRIDGE_CLASSIFICATION_METHODOLOGY.md` and `data/PROVENANCE.md` updated.  No shipped-data change.
- **Pericoupling four-stage rebuild — parameter-free core + source-relabel + HydroLAKES completeness.**  The adjacency database is now constructed by a four-stage pipeline that supersedes the counts of every entry below.  **(1) Topology** — exact-contact rook contiguity on the World Bank geometry alone at **tolerance 0** (`TOPOLOGY_TOL_DEG = 0.0`; the former 5×10⁻⁴° snap survives only as the disputed-overlay tract-touch test), full geodesic border lengths, and **lakes as native edges**: the `ne_10m_lakes` inland-water filter is removed, so `coupling_standard` governs lakes and rivers uniformly with no restoration overlay (`lake_overlay_pairs.csv` deleted; NE lakes are now used for descriptive water-type classification only).  Before contiguity, a new **source-relabel stage** (`scripts/relabel_sliver_corridors.py` + reviewed manifest `data/sliver_corridor_relabel.csv`) reassigns 11 thin border-tracing "sliver-corridor" artifacts across 10 WB host polygons to their true same-country owners by morphological opening — area-conserving, removing every fabricated cross-country edge (e.g. Migori↔Arusha) and recovering the starved real ones (Migori↔Mara 20 → 103 km); the three genuine panhandles (Vennbahn, Courantyne, Dhekelia) are deliberately untouched.  **(2) De-facto connectivity** — the NDLSA disputed-territory overlay (+13 ADM1 / +3 ADM0), unchanged.  **(3) Water classification** — purely descriptive (~2.5 km river-centerline screen; never adds or drops an edge).  **(4) Second-source completeness** — the reviewed overlay manifests, joined by two new ones: `data/hydro_lakes_overlay_pairs.csv` (12 human-verified lake reclassifications — incl. the Dead Sea, folded in from the since-deleted hydro-geodesic lake overlay — from a full-database **HydroLAKES** sweep of all shipped edges — Great Lakes, Lake Malawi median line, Lake Chad, Titicaca, Victoria, Kivu…) and `data/lake_gap_overlay_pairs.csv` (the only two non-touching lake borders: `EST006`↔`RUS061` across Lake Peipus, `ALB022`↔`MNE002` across Lake Skadar); the land-gap overlay shrinks to 5 pairs (the two Kenya–Tanzania survey-line pairs are native at tolerance 0).  The base bridge classification was corrected 312 → **309** rows (Vistula Spit and Konstanz–Kreuzlingen demoted to mixed land borders; one orphan row removed after the relabel deleted its edge) and 10 reservoir names added to `water_body`.  Every stage is a deterministic Python screen whose frozen, human-adjudicated manifests the build replays — no live AI at build time.  Counts: ADM1 **8,451** shipped = 8,425 native + 13 de-facto + 13 overlay-restored (moderate **8,214** / stringent **8,090**), regions **3,375**, countries **196**; water-only **361** ADM1 (**124** with an open fixed crossing / **237** without; 285 river / 76 lake) + **22** ADM0 roll-ups (17/5); ADM0 **326** (moderate **321** / stringent **304**).  Full number-by-number ledger: `docs/METHODS_adjacency.md` §"Construction and the source of every number"; verification: `python scripts/build_all.py`.  (#76)
- **Hydro-water overlay: 18 human-verified water-only borders the Natural Earth screens missed — incl. the Uruguay River, folded in from the since-deleted hydro-geodesic river overlay — (full-database HydroRIVERS cross-check) + 3 base-classification corrections.**  The S2 NE-vs-HydroRIVERS comparison had only ever covered the 48 near-miss pairs; this audit ran it over **all 1,800 cross-border edges** (each border sampled every ~0.5 km against HydroRIVERS v10 within a ~500 m buffer, at discharge tiers 0/10/100 m³/s; `build_data/hydro_full_sweep/`).  36 miss-candidates + 3 disputed pairs were flagged, the 34 un-adjudicated ones two-stage ground-truthed, and **every proposal human map-verified (2026-07-02)**: 17 genuine water-only borders — incl. the **Oder/Neisse** (Brandenburg↔Lubuskie, bridged), **Río Hondo** (Corozal↔Quintana Roo, bridged), **Cavally** (3 pairs), **Mano** (2 pairs), **Kagera** (Muyinga↔East), **Alazani** (Kakheti↔Gakh), **Una/Sava**, **Mur/Kučnica**, **Madeira/Abuná**, **Ireng**, **Ntem**, **Djoua**, **Meli/Moa**, **Maroni/Lawa/Litani** — shipped as `data/hydro_water_overlay_pairs.csv` (flags only, no new edges; 5 bridged / 12 not, 6 / 12 once the Uruguay River was folded in).  The audit also **corrected the base bridge classification**: Pskov↔Ida-Viru reclassified river→lake (the border runs through Lake Peipus), and Osijek-Baranja↔Bács-Kiskun + Eastern Equatoria↔Moyo removed as not water-only (human-verified) — base set 314 → **312** rows, SHA-256 re-pinned; one flag (Comoé↔Western, Tano) human-rejected over the verifier's confirmation.  New ADM0 roll-up **GUF↔SUR** (Maroni, ferry only).  Counts: water-only 337 → **352** (121/231) + 22 ADM0 roll-ups; ADM1 moderate 8,232 → **8,222**, stringent 8,116 → **8,101**; ADM0 moderate 323 → **322**, stringent 305 → **304**; shipped edges 8,453, regions, and the ADM0 matrix unchanged.
- **One-command reproducible database build: `scripts/build_all.py` + unified overlay engine `scripts/apply_overlays.py`.**  The pericoupling database's construction is now cleanly separated from its discovery history: one deterministic geometry build (inputs SHA-256-pinned in `data/PROVENANCE.md` and verified by `--full`) plus one reviewed **correction layer** — the five overlay manifests (88 pair-rows: 72 edge-restoring + 16 water-flag-only) — applied in a single idempotent pass by the new engine (fixed registry order; water rows and the ADM0 roll-up computed once, eliminating the former order-fragility where three scripts truncated `water_separated_pairs.csv`).  `python scripts/build_all.py` re-derives and verifies every headline count from the shipped data; `--full` rebuilds from the pinned GeoPackages.  The engine reproduces the shipped CSVs **byte-identically** from the raw build state and is a byte-stable no-op on shipped data (guarded by the new `tests/test_apply_overlays.py`).  The five per-overlay apply scripts (`apply_river_gap/wide_river/lake/land_gap/audit_water_overlay.py`) are superseded and removed; the manifests are now the authoritative correction data.  `docs/METHODS_adjacency.md` and `data/PROVENANCE.md` restructured to lead with construction and present the audits as the discovery/validation record.  Shipped data unchanged (byte-identical).
- **Audit-water overlay: three screen-missed water-only borders, human-verified (Shirak↔Kars, Vratca↔Olt, Kagera↔Ntungamo).**  The 10 km completeness sweep's verification was completed with a second adversarial pass over the 78 single-pass candidates, so **all 125** candidates now carry two independent verdicts (verdict flips on 11/125 ≈ 9% — ~17% within the borderline 47, ~4% among the rest; artifacts in `build_data/wide_river_audit/`).  The **12** pairs with a flip or a positive verdict were adjudicated by **human map review (2026-07-02)**: nine confirmed mixed river+land (status quo), and three confirmed genuine water-only borders the Natural Earth screens missed — **Shirak↔Kars** (`ARM007`↔`TUR045`, ~109 km on the Akhurian/Arpaçay incl. its reservoir; `has_bridge=True` — a completed crossing on a politically closed border counts), **Vratca↔Olt** (`BGR028`↔`ROU031`, a 17.4 km Danube main-stem span entirely absent from the NE river list, confirmed against HydroRIVERS at 90 m median offset; ferry only), and **Kagera↔Ntungamo** (`TZA006`↔`UGA094`, a 402 m Kagera-thalweg arc at the RWA-UGA-TZA tripoint; no crossing).  Shipped as `data/audit_water_overlay_pairs.csv` + idempotent `scripts/apply_audit_water_overlay.py`; all three are existing edges, so the overlay adds water-only flags only (no new edges).  Counts: water-only set 334 → **337** (116 with a fixed crossing / 221 without); ADM1 moderate 8,234 → **8,232**, stringent 8,119 → **8,116**; shipped edges **8,453**, regions, and the ADM0 matrix unchanged (Shirak↔Ardahan keeps ARM↔TUR mixed at ADM0).
- **Land-gap overlay: two survey-line borders the offset corridor hid (Kajiado↔Kilimanjaro, Narok↔Mara).**  Auditing all **87** pairs a looser snap tolerance would add (recovered-length diagnostic per pair; `build_data/snap_extras_audit/`) confirmed every one shows the gap-corridor signature and every cross-border case lies inside the ground-truthed near-miss set — and surfaced two long offset corridors on the straight-surveyed Kenya–Tanzania border whose dry-land adjacency lay outside the earlier river audit's scope: **Kajiado↔Kilimanjaro** (`KEN010`↔`TZA011`, ~54 km, Loitokitok–Rombo) and **Narok↔Mara** (`KEN033`↔`TZA016`, ~70 km, Maasai Mara–Serengeti), both within ~100 m gaps (the two national renderings of the same survey line).  Human map verification confirmed both as genuine land borders; they are restored by the **land-gap overlay** (`data/land_gap_overlay_pairs.csv`, applied idempotently by `scripts/apply_land_gap_overlay.py`).  Unlike the water overlays these are ordinary land edges, pericoupled under every `coupling_standard`.  Counts: ADM1 8,451 → **8,453** shipped (moderate 8,232 → **8,234**, stringent 8,117 → **8,119**); ADM0, regions, and the water-only set unchanged.
- **ADM1 name resolver: transliterate letters NFKD cannot decompose.**  `_fold_diacritics` now maps the handful of standalone letters that Unicode NFKD cannot decompose into letter + combining mark (Ł/ł, Đ/đ, Ø/ø, Ð/ð, Þ/þ, Æ/æ, Œ/œ, ß, ı) to their conventional ASCII transliterations, so native-script queries such as `Łódź` → `POL003` and `Đắk Lắk` → `VNM016` resolve the ASCII names the database stores (previously they returned `None` while their ASCII forms worked).  Ordinary combining-mark folding (`Jõgeva`→`Jogeva`, `Michoacán`→`Michoacan`) is unchanged.  Regression test added.
- **Lake-gap addition: Jõgeva↔Pskov across Lake Peipus.**  The near-miss audit's one wide-lake case — Jõgeva maakond (Estonia) ↔ Pskovskaya Oblast (Russia), whose shores the World Bank source digitizes as non-touching (gap ~0.0 km, facing arc ~0.5 km, lake fraction 1.00 in both river layers) — was manually verified as a genuine mid-lake border and added to the lake overlay as its 64th pair (`water_type=lake`, no fixed crossing → pericoupled under `lenient` only).  This closes the last audited residual case of the near-miss audit.  Counts: ADM1 shipped edges 8,450 → **8,451** (moderate 8,232 and stringent 8,117 unchanged); water-only set 333 → **334** (115 with a fixed crossing / 219 without); ADM0 unchanged.
- **Lake overlay: `coupling_standard` now governs lake borders exactly like river borders.**  The geometry build's lake filter drops pairs that meet only across a lake (mid-Great-Lakes states, Lake Victoria districts), which made the water-only policy river-only in practice: a *bridged* lake pair (e.g. Flevoland↔Noord-Holland via the Houtribdijk) could never be pericoupled, because the loaders only subtract from the base graph — the long-documented Flevoland limitation.  The reviewed **lake overlay** (`data/lake_overlay_pairs.csv`, applied by `scripts/apply_lake_overlay.py`) restores all **63** audited lake-only ADM1 pairs (the Natural Earth lake-filter candidate set; every pair carries the OSM + web + manual `has_bridge` review from the bridge-classification pipeline) as edges, so the three standards apply uniformly to water borders of both kinds: `lenient` keeps every water border, `moderate` (default) keeps only those with a fixed crossing open to traffic, `stringent` keeps none.  Of the 63, **3** are bridged and so join the default view — Flevoland↔Noord-Holland (Houtribdijk N302 / Stichtse Brug A27 / Flevolijn rail), Flevoland↔Gelderland (Nijkerkerbrug), and Södermanland↔Uppsala (Hjulstabron, Riksväg 55 across Mälaren) — resolving the Flevoland limitation; Flevoland↔Utrecht correctly stays lenient-only (the Stichtse Brug lands in Noord-Holland).  The ADM0 matrix gains the one country pair whose only contact is a lake: `COD`↔`TZA` across Lake Tanganyika (lenient-only).  Counts: ADM1 shipped edges 8,387 → **8,450** (moderate 8,229 → **8,232**, stringent **8,117** unchanged), regions 3,373 → **3,375** (the island districts Kalangala and Buvuma gain their first edges); ADM0 shipped 325 → **326** (moderate **323** and stringent **305** unchanged); water-only roll-ups 20 → **21** ADM0.  Two guard tests and a lake-behavior test added.
- **Wide-river overlay: 13 reviewed water-only ADM1 pairs the 2.4 km screen missed.**  The water-only candidate screen flags a border when >60% of it lies within ~2.4 km of a major Natural Earth river centerline; this under-counts *very wide* rivers (chiefly the Danube), whose World-Bank border weaves >2.4 km from the mapped centerline.  Re-screening the candidate net at **5 km** and ground-truthing every new candidate per-pair (LLM research + adversarial verification) recovered **13** genuine water-only pairs — 8 Bulgaria↔Romania Danube spans plus the Ruvuma (Niassa↔Ruvuma), Bojana (Shkodër↔Ulcinj), Moselle (Rheinland-Pfalz↔Grevenmacher), Tumen (Hamgyŏng-bukto↔Primorsky Kray), and Zambezi/Kazungula (Chobe↔Southern); 54 other 5 km candidates were rejected as mixed river+land, vindicating the conservative screen.  They are restored as the reviewed **wide-river overlay** (`data/wide_river_overlay_pairs.csv`, applied by `scripts/apply_wide_river_overlay.py`); all are existing edges, so the overlay adds a water-only flag only (the **8,387** edge total is unchanged).  Of the 13, 6 have a fixed crossing (kept under `moderate`) and 7 are ferry-only.  One of the six bridge calls rests on domain confirmation rather than web verification alone: public documentation of the Niassa↔Ruvuma crossing (Unity Bridge 2, Mkwenda/Matchedje) is conflicting, and the audit's initial conservative call was `has_bridge=False`; the shipped `has_bridge=True` follows domain confirmation (2026-06-29) that at least one fixed crossing exists on this border.  Shipped water-only set 320 → **333** (115 with a fixed crossing / 218 without); default-view edges ADM1 8,236 → **8,229** (moderate) and 8,130 → **8,117** (stringent); ADM0 water-only roll-up 17 → **20**, stringent 308 → **305** (moderate/lenient unchanged).  Full method + audit in `paper/SUPPLEMENT_river_border_audit.md`.
- **River-gap overlay: 6 reviewed cross-border river-separated ADM1 pairs.**  The strict rook-contiguity build omits cross-border pairs separated by a river when the World Bank source digitizes the two opposite banks as non-touching polygons.  An audit of all 48 cross-border near-miss pairs (within ~1 km, not touching) — a Natural-Earth-vs-HydroRIVERS cross-check on all 48, with the 29 pairs either river layer flags as water or maritime individually ground-truthed against authoritative border geography and the other 19 (dry slivers on which both river layers and the build agree there is no water feature) accepted as correctly omitted on that layer agreement — recovered **six** genuinely omitted river-separated pairs (Galați↔Odesa and Tulcea↔Odesa across the Danube, Caraș-Severin↔Borski across the Danube at the Iron Gates, Cahul↔Vaslui across the Prut, Saarland↔Grevenmacher across the Moselle, French Guiana↔Marowijne across the Maroni).  They are restored as a reviewed authored overlay (`data/river_gap_overlay_pairs.csv`, applied by `scripts/apply_river_gap_overlay.py`), analogous to the disputed-territory overlay, so the geometry-derived base stays pure.  All six are water-only pairs governed by `coupling_standard` (only the bridged Moselle pair is exposed under the default `moderate`).  This raises the shipped ADM1 graph to **8,387** pairs (**8,236** under moderate) and the water-only set to **320** (109 with a fixed crossing / 211 without); the ADM0 matrix is unchanged.  Three further audited cases were already represented under their correct ADM1 units (the flagged near-miss pair was a tripoint/wrong-unit artifact), and the remaining near-miss pairs are correctly omitted (tripoint corners, sea straits, dry-land/internal boundaries, and one wide-lake case).  Full audit in `paper/SUPPLEMENT_river_border_audit.md`.

- **`de_facto_borders` / `coupling_standard` are now configurable on `MetacouplingAssistant`.**  The post-analysis pericoupling validators previously hardwired the database defaults; the assistant now accepts `de_facto_borders` (default `True`) and `coupling_standard` (`"stringent"`/`"moderate"`/`"lenient"`, default `"moderate"`) constructor params and threads them into both the country-level and ADM1 validators.  So a study using a stricter or looser border policy gets validation that matches: e.g. with `coupling_standard="stringent"` a river-only neighbour (Romania↔Moldova) is validated as telecoupled rather than pericoupled.  An invalid `coupling_standard` raises `ValueError` at construction.

- **Built-in run tracing for `MetacouplingAssistant` (on by default).**  Every `analyze()` / `refine()` now captures a `RunTrace` and writes a folder of human-readable artifacts (`00_run_config.md` … `10_pipeline_metadata.md` + `README.md` + `map.png`) — the same layout the `scripts/trace_pipeline.py` harness used to produce externally, now produced by the package itself.  The trace records every model call (prompts, response, token usage, duration), the pipeline intermediates (raw web results, RAG chunks, parsed analysis, map data, formatted output), and run metadata (model, git SHA, environment).  It is attached to the result as `result.trace` (a `RunTrace`); `trace.out_dir` is the artifact folder.  Controlled by two constructor args: `trace` (default `True`; set `False` to disable, or the `METACOUPLINGLLM_DISABLE_TRACE` env var) and `trace_dir` (default `runs/<utc>_<slug>/`, gitignored; multi-turn runs write `turn1/`, `turn2/` …).  Tracing never raises into the caller — a write failure leaves the analysis result intact with `trace.out_dir=None`.  Internally the LLM client is wrapped in a recording proxy, so all providers are captured uniformly; `scripts/trace_pipeline.py` is now a thin driver over this built-in.  This also makes the package's reproducibility/auditability story real rather than harness-only.
...
- **Trace `README.md` now documents the artifact-numbering convention.**  Each run's generated `README.md` gained a "Reading this trace" note explaining that files are numbered by *pipeline stage* (the order steps run), not by model-call order — so the structured web-extraction call is `02` because it runs during the web-search stage, before the main analysis — and that the `LLM calls` count and the `10_pipeline_metadata.md` token table include only calls captured through the assistant's `chat()` proxy (the web-extraction call is *summarized*, with raw inputs/outputs in `01`/`03`, not chat-captured).  The artifact table's previously-empty Description column is now filled in per file.
...
- **Clarified the RAG corpus description (full text vs structured summaries).**  The INTRODUCTION described the 420-paper corpus as "full text for the 296 open-access papers, structured summaries for the 124 non-open-access papers", which read as if all 296 OA papers were full text.  In fact the corpus splits by what can be *redistributed*, not by topic or OA status: **192 papers are indexed as full text** and the **other 228 as structured paraphrased summaries** (metadata + key findings, no source text) — those 228 being papers that cannot be redistributed in full, i.e. paywalled articles (124) *and* open-to-read articles under restrictive licences (104).  Reworded the three INTRODUCTION mentions (feature list, §4.3, bundled-data inventory) accordingly.  Also **introduced the RAG corpus in the MANUAL** for the first time — a new "What's in the RAG corpus" note in §5 (size, full-text/summary split, pointer to PROVENANCE) plus a clarifier in §9 — since the MANUAL previously described only the separate 265-article recommendation database, which a reader could mistake for the RAG corpus.  (#63)
...
- **`de_facto_borders` toggle for disputed-territory adjacency.**  The
  ...
  **no** ADM1 pair: Gilgit-Baltistan and Ladakh/J&K are disputed territories
  ...
  audit in `docs/ndlsa_tract_audit.csv`.  Attributions are authored (the source's
  sovereignty field is empty for every tract) and encode physical coupling, **not** a legal claim —
