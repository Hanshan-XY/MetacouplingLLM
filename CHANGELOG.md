...
- **HydroRIVERS screens sample by geodesic length; three unnominated rows leave the water-only set (water-only 808 → 805 = 408 / 397; moderate 8,059 → 8,061; stringent 7,650 → 7,653).**  The three HydroRIVERS-family screens (domestic sweep, cross-border sweep, creek-band enrichment) walked the shared border at a fixed 4.5×10⁻³° of planar length — about 500 m north–south but only 500·cos(latitude) m east–west — so their coverage was a direction-weighted share; the Natural Earth and HydroLAKES screens had always sampled by geodesic length.  On the maintainer's decision (campaign `gs1`) all three now sample at intervals of at most 500 m of geodesic length.  Re-run on the pinned inputs, the change reshuffles bar-edge cases in both directions (median coverage change about zero).  **Nineteen creek-band pairs were newly nominated** with no adjudication record (17 domestic, 2 cross-border) and went through the standard two-pass — blind GPT-5.6 Sol research, Sonnet-5 adversarial judgment, gate A 0 / B 1 / C 6 / D 12 — and the maintainer ruled the one disagreement (Vitebsk↔Zilupes novads) not water-only after a gap measurement: all 19 rejected.  **Three shipped rows were no longer nominated by any screen** (Lekoumou↔Niari 0.50 → 0.49; Kyegegwa↔Ssembabule 0.50 → 0.33 on a 0.65 km border; Hai Duong↔Quang Ninh 0.51 → 0.49 — creek-band rows ruled water-only on maps on 2026-09-14) and, on the maintainer's decision, leave the water-only set: the database is complete relative to its screens, and a row no screen nominates falls outside the method.  That is **not a finding that they are land**; the edges stay as ordinary land borders and the three are recorded as known omissions in `docs/FUTURE_WORK.md` §4.  Counts: water-only 805 = 408 with a fixed crossing / 397 without; river 691 (396 / 295); rescreen-water 483 (298 / 185); creek-band rows 51 (47 / 4); tier A 270; moderate 8,061; stringent 7,653; lenient 8,458 and every ADM0 count unchanged (all three are domestic).  Completeness 0 open; whole-graph attributability 805/805.  Also fixed in the working archive: the creek-band enrichment script, which a later comment edit had left unparseable.  Record: `build_data/geodesic_sampling/`.
