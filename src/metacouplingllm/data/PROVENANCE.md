...
three mid-lake point/corner contacts removed 2026-07-25 by wu1 maintainer map
...
DEU↔LUX, BEN↔NER, CMR↔GAB, and MWI↔TZA bridged, GUY↔SUR ferry-only, the one
new moderate-view subtraction).
...
2026-09-22 — and the union-mask corridor census for non-touching pairs) nominate
all 803 shipped water rows — 298/298 base rows, 15/15 widening rows, 18/18
...
- **Water-separated pairs (`coupling_standard`).** 803 ADM1 pairs (and 26
  ...
  **320** moderate / **300** stringent). Each pair's `has_bridge` flag was classified
  ...
  method + per-pair sources: `docs/BRIDGE_CLASSIFICATION_METHODOLOGY.md`. One post-classification correction: Italy↔Vatican (`ITA007`↔`VAT001`) was review-reclassified from water-only to a **land border** (the Vatican is a ~2.7 km land enclave within Rome; the Tiber is ~0.6 km away, not the border), so the land-classified subset is **314** ADM1 water-only pairs, not the 315 first geometrically flagged (the shipped total is **803** after the hydro-water, hydro-lakes, and rescreen overlays below; the land-classified subset itself is **309** after the 2026-07-02 corrections below). ITA↔VAT is therefore pericoupled under every `coupling_standard`.
...
- **Lakes: native edges under the uniform water policy.** The topology build
  applies **no lake filter** (`scripts/build_pericoupling_db.py`), so the ~56
...
- **Collapsed-node (quadripoint) artifacts.** At a few places the WB source
  ...
  ≥4-province single-point nodes returns 33 candidates (30 domestic,
