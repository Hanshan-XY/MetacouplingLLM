...
1. **Topology** (`scripts/build_pericoupling_db.py`): rook contiguity at **exact
   ...
   **source-relabel** step first fixes 10 reviewed WB sliver-corridor artifacts
   ...
   contacts removed 2026-07-25 on the maintainer's official-map
...
3. **Water classification** (descriptive only): the reviewed bridge
   ...
   layer); never adds/drops an edge.
4. **Second-source completeness** (`scripts/apply_overlays.py`): the reviewed
   ...
   re-screens, HydroRIVERS/HydroLAKES sweeps; since 2026-09-22 the edge screens
   run on one border arc, `build_data/arc_and_combined_screens/`, and since
   2026-09-23 every screen distance is measured on the ground,
   `build_data/geodesic_distances/`) before any
   human/AI adjudication,
...
Rostov/Stavropol frontages into the existing RUS024 rows) = 8,422, −5 denylist
(the audit-confirmed LBR006↔LBR014 and VEN001↔VEN003 artifacts, plus the
...
the rescreen-water overlay completes six more country borders as all-water:
...
Nor are the pre-rebuild nets nomination steps any longer (retired 2026-09-09):
...
all 803 shipped water rows — 298/298 base rows, 387/387 rows of the 2026-07
rebuild (383 from its batches b1–b6 and 20 km holds, 3 placeholder-identity
...
All ran *downstream* of the deterministic screens. The pre-rebuild overlays (hydro-water,
...
verification is retained rather than replaced; the rescreen overlays carry human
...
rescreen rows — 387 + the 15 ru1-folded river rows, which carry pre-rebuild
...
- **Tier C — reviewed base classification:** the 298-row bridge-classified
...
- `verification_tier` — the **evidence** strength: `A` / `B` / `C` above,
  currently **268 / 238 / 297** (tier A: 243 rescreen-water rows, all 24
...
| `rescreen_water_overlay_pairs.csv` | 481 ADM1 pairs | Reviewed rescreen-water manifest (water-screen rebuild, batches b1–b6 + the 20 km-hold tranche; the 14 domestic large-river rows of 2026-09-01 (15 accepted; Montevideo↔San José left the water-only set on 2026-09-22 when no screen nominated it on the border arc) and the 50 domestic creek-band rows of 2026-09-14 (54 accepted; three left the water-only set on 2026-09-21 when, after the HydroRIVERS screens took the other screens' sampling rule, no screen nominated them any more, and two more on 2026-09-22 under the border arc, while Lekoumou↔Niari, nominated again, was re-adjudicated and returned, record `build_data/arc_and_combined_screens/`) — the HydroRIVERS creek band's 715 domestic creek-only edges (714 set aside on 2026-07-10 as document-only, and one edge no screen population held), adjudicated by the standard cross-vendor two-pass, a deterministic gap measurement and maintainer map rulings, then the four-layer bridge pipeline, 47 bridged / 7 not at adjudication and 48 / 6 after the crossing unification, 45 / 5 as shipped since 2026-09-22, tier A, record `build_data/water_screen_rebuild/domestic_creek/`; the one row of the 2026-09-18 pilot re-adjudication, Kapisa↔Parwan along the Panjshir River, accepted by maintainer map ruling after the nine-pair pilot's single-vendor verdicts were replaced by the cross-vendor two-pass, bridged, tier A, record `build_data/water_screen_rebuild/pilot_readjudication/`; since 2026-07-28 also carrying the folded 18 hydro-water + 12 hydro-lakes rows — HydroRIVERS/HydroLAKES geodesic 500 m full-database cross-checks, incl. the Uruguay River and the Dead Sea — each with its discovery provenance in `source`): existing edges — cross-border **and domestic** — reclassified water-only by the rebuilt full-ladder screens (on one border arc since 2026-09-22) and the Tier-2 two-pass audit (GPT-5.6 Sol research → Sonnet 5 adversarial judgment across all batches; 9 rows ruled by the maintainer on the shipped WB geometry, 2026-09-19; human/dual-AI verification throughout; per-row provenance in `source`). Flags only, per-row `water_type`. |
| `rescreen_gap_overlay_pairs.csv` | 24 ADM1 pairs | Reviewed rescreen-gap manifest (water-screen rebuild, batch b1, plus the 5 river rows and the Skadar lake row folded in on 2026-07-22/23 from the retired river-gap and lake-gap overlays, all re-nominated by the corridor census; plus the 2 rows the corridor census v2 campaign added on 2026-09-10 — Équateur↔Cuvette, Entre Ríos↔Artigas — tier A, maintainer map-ruled): non-touching water borders the source digitizes as separate banks/shores, restored as edges (e.g. the Gambia River, the Elbe, Lac de Biaufond, the Vietnamese Mekong distributaries); `border_km` = geodesic facing-arc length; per-row `water_type`, bridge flags from the four-layer bridge pipeline. |
...
- **Water-separated pairs (`coupling_standard`).** 803 ADM1 pairs (and 26
  ...
  **320** moderate / **300** stringent). Each pair's `has_bridge` flag was classified
  ...
  method + per-pair sources: `docs/BRIDGE_CLASSIFICATION_METHODOLOGY.md`. One post-classification correction: Italy↔Vatican (`ITA007`↔`VAT001`) was review-reclassified from water-only to a **land border** (the Vatican is a ~2.7 km land enclave within Rome; the Tiber is ~0.6 km away, not the border), so the land-classified subset is **314** ADM1 water-only pairs, not the 315 first geometrically flagged (the shipped total is **803** after the hydro-water, hydro-lakes, and rescreen overlays below; the land-classified subset itself is **309** after the 2026-07-02 corrections below and **298** as shipped, see *Sources*). ITA↔VAT is therefore pericoupled under every `coupling_standard`.
...
- **River-separated province pairs (river-gap overlay).** Under the strict
  ...
  two-pass + maintainer map rulings**)** that overlay was folded into the
  rescreen-gap manifest (registry entry `rescreen_gap`): five rows ship there
  with rg1 provenance — Galați↔Odesa upheld by maintainer map ruling (adjacent
...
- **Wide-river overlay (5 km re-screen).** The 2.5 km river-centerline candidate
  ...
  cross-vendor process and folded into the rescreen-water manifest; the
  overlay and its registry entry are retired. All are existing edges — flags
  only, no new edges.
...
- **Land-gap overlay (survey-line offset corridors).** Along straight-surveyed
  ...
  registry entry `land_gap`) ships **4** unrelated pairs from the tolerance-0
...
- **Hydro-water overlay (full-database HydroRIVERS cross-check).** The S2
  ...
  the hydro-water rows (flags only, 5 bridged / 12 not; carried in
  `rescreen_water_overlay_pairs.csv` since the 2026-07-28 consolidation,
  which followed the cross-border extension of the rebuilt hydro rungs —
  ...
  ordinary land edges, pericoupled under every standard; the bridge CSV went to
  309 rows (SHA-256 re-pinned; 301 after the ru1 demotions, 2026-07-21, and 298 as
...
- **Rescreen overlays (2026-07 water-screen rebuild, batches b1–b6 + the
  ...
  (420 pairs) ships **72 rescreen-water flags** (71 remain after the
  validation-study demotion of NIC016↔NIC017) and **16 rescreen-gap edges**;
  batch b2 (393 pairs) ships **125 rescreen-water flags** (incl. 9 on a
...
- **Sub-tolerance digitization offsets (bounded by the sensitivity sweep).**
  ...
  (`docs/METHODS_adjacency.md` §2.4, §4). The
  `_ADM1_FALSE_POSITIVE_DENYLIST` holds five reviewed entries — the
