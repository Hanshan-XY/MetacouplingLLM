...
| File | Shape | Content |
|---|---|---|
| `pericoupled_adm1_edge_list.csv` | 8,387 edges · 3,373 ADM1 regions · 196 countries | Subnational (ADM1) shared-land-border adjacency |
...
| `water_separated_pairs.csv` | 333 ADM1 + 20 ADM0 pairs | Water-only pairs + `has_bridge` for the `coupling_standard` filter (314 land-classified + 6 river-gap + 13 wide-river overlay; see *Water-separated pairs* below) |
...
- **Water-separated pairs (`coupling_standard`).** 333 ADM1 pairs (and 20
  ...
  `stringent` drops every water-only pair (ADM1 default-view edges 8,387 →
  **8,229** moderate / **8,117** stringent; ADM0 325 → **323** moderate /
...
- **Wide-river overlay (5 km re-screen).** The 2.4 km river-centerline candidate
  ...
  adds a water-only flag only (the 8,387 edge total is unchanged). This brings the
  shipped water-only set to **333** ADM1 (115 with a fixed crossing / 218 without)
  and **20** ADM0 roll-ups; default-view edges become ADM1 8,229 (moderate) /
  8,117 (stringent) and ADM0 323 (moderate) / 305 (stringent).
...
- **Snapping-tolerance false positives (manually removed).** The snap tolerance
  ...
  **`MLT002`/`MLT019`** (Balzan / Iklin, Malta; polygons ~31 m apart) — is
  ...
  for the shipped total of **8,387**; see *Disputed territories* above). A
