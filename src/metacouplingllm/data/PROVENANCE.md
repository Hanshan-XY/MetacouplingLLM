...
map ruling — the true contact is a point) = 8,434, +22 rescreen-gap
(water-screen rebuild, batch b1, plus the rg1 river-gap fold 2026-07-22 and
the lg1 lake-gap fold 2026-07-23) = **8,456** shipped
(lenient); **8,061** moderate; **7,705** stringent; 3,374 regions, 196 countries.
**ADM0** 326 / 320 / 300. **Water-only 751** (356/395) + 26 ADM0 roll-ups
...
751 shipped water rows — 298/298 base rows, 15/15 widening rows, 18/18
hydro-water and 12/12 hydro-lakes rows, 6/6 near-miss/lake-band rows
...
- `adjudication` — the **process** class that produced the verdict. Uniformly
  `cross-vendor` across all 751 rows: ru1 (2026-07-21) covered the river class,
...
| File | Shape | Content |
|---|---|---|
| `pericoupled_adm1_edge_list.csv` | 8,456 edges · 3,374 ADM1 regions · 196 countries | Subnational (ADM1) shared-border adjacency (land borders + the water-only overlay edges below) |
...
| `water_separated_pairs.csv` | 751 ADM1 + 26 ADM0 pairs | Water-only pairs + `has_bridge` for the `coupling_standard` filter (298 base + 431 rescreen-water + 22 rescreen-gap; see *Water-separated pairs* below) |
...
| `rescreen_water_overlay_pairs.csv` | 431 ADM1 pairs | Reviewed rescreen-water manifest (water-screen rebuild, batches b1–b6 + the 20 km-hold tranche; since 2026-07-28 also carrying the folded 18 hydro-water + 12 hydro-lakes rows — HydroRIVERS/HydroLAKES geodesic 500 m full-database cross-checks, incl. the Uruguay River and the Dead Sea — each with its discovery provenance in `source`): existing edges — cross-border **and domestic** — reclassified water-only by the rebuilt full-ladder screens and the Tier-2 two-pass audit (Codex research → Sonnet 5 adversarial judgment across all batches; 9 rows shipped on human-delegated Fable-5 final arbitration; human/dual-AI verification throughout; per-row provenance in `source`). Flags only, per-row `water_type`. |
| `rescreen_gap_overlay_pairs.csv` | 22 ADM1 pairs | Reviewed rescreen-gap manifest (water-screen rebuild, batch b1, plus the 5 river rows and the Skadar lake row folded in on 2026-07-22/23 from the retired river-gap and lake-gap overlays, all re-nominated by the corridor census): non-touching water borders the source digitizes as separate banks/shores, restored as edges (e.g. the Gambia River, the Elbe, Lac de Biaufond, the Vietnamese Mekong distributaries); `border_km` = geodesic facing-arc length; per-row `water_type`, bridge flags from the four-layer bridge pipeline. |
...
> table above (authoritative counts: 8,456 / 8,061 / 7,705; water-only 751);
...
- **Water-separated pairs (`coupling_standard`).** 751 ADM1 pairs (and 26
  ...
  shipped edges 8,456 → **8,061** moderate / **7,705** stringent; ADM0 326 →
  **320** moderate / **300** stringent). Each pair's `has_bridge` flag was classified
  ...
  method + per-pair sources: `docs/BRIDGE_CLASSIFICATION_METHODOLOGY.md`. One post-classification correction: Italy↔Vatican (`ITA007`↔`VAT001`) was review-reclassified from water-only to a **land border** (the Vatican is a ~2.7 km land enclave within Rome; the Tiber is ~0.6 km away, not the border), so the land-classified subset is **314** ADM1 water-only pairs, not the 315 first geometrically flagged (the shipped total is **751** after the hydro-water, hydro-lakes, and rescreen overlays below; the land-classified subset itself is **309** after the 2026-07-02 corrections below). ITA↔VAT is therefore pericoupled under every `coupling_standard`.
...
- **Lakes: native edges under the uniform water policy.** The topology build
  ...
  **seven** lake pairs with a fixed crossing open to traffic
  ...
  totals to **8,456** / **751** (356/395) / **26** roll-ups.
...
- **Rescreen overlays (2026-07 water-screen rebuild, batches b1–b6 + the
  ...
  counts: ADM1 **8,456** edges (**8,061** moderate / **7,705** stringent),
  ADM0 **326** (320 / 300), water-only **751** ADM1 (356/395) + **26** ADM0
