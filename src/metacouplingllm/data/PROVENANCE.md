...
2. **Reviewed correction layer** (`scripts/apply_overlays.py`): the six overlay
   manifests below — **105 pair-rows** (72 edge-restoring + 33 water-flag-only),
   ...
   audit-water → hydro-water; water rows and the ADM0 roll-up are computed once, at the end).
...
| `water_separated_pairs.csv` | 352 ADM1 + 22 ADM0 pairs | Water-only pairs + `has_bridge` for the `coupling_standard` filter (312 land-classified + 6 river-gap + 13 wide-river + 1 lake-gap + 3 audit-water + 17 hydro-water overlay; see *Water-separated pairs* below) |
...
| `hydro_water_overlay_pairs.csv` | 17 ADM1 pairs | Reviewed hydro-water overlay manifest: water-only borders the Natural Earth screens missed, from the full-database HydroRIVERS cross-check of all 1,800 cross-border edges, human-verified (flags only; see *Hydro-water overlay* below) |
...
- **Bridge classification** — `build_data/bridge_classified_authoritative.csv`
  ...
  `dd20e3f29c5d6e83949dbefb9d7d0a641237b94960ad2195e0bf123f5224576b`.
...
- **Lake overlay (uniform water policy).** The geometry build's lake filter
  ...
  stays lenient-only: the Stichtse Brug lands in Noord-Holland). Shipped counts
  after this overlay: ADM1 **8,453** edges, ADM0 **326** pairs, water-only
  **334** ADM1 (115/219) + **21** ADM0 roll-ups; final views follow the
  audit-water overlay below.
