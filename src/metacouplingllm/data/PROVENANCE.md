...
| `water_separated_pairs.csv` | 333 ADM1 + 21 ADM0 pairs | Water-only pairs + `has_bridge` for the `coupling_standard` filter (314 land-classified + 6 river-gap + 13 wide-river overlay; see *Water-separated pairs* below) |
| `lake_overlay_pairs.csv` | 63 ADM1 pairs | Reviewed lake overlay manifest: the audited lake-only pairs (Natural Earth lake-filter candidate set, per-pair bridge review) restored as edges by `scripts/apply_lake_overlay.py` so `coupling_standard` governs lakes like rivers (see *Lake overlay* below) |
...
- **Water-separated pairs (`coupling_standard`).** 333 ADM1 pairs (and 20
  ...
  borders since the lake overlay below (ADM1 shipped edges 8,450 →
  ...
  **305** stringent). Each pair's `has_bridge` flag was classified
  ...
  method + per-pair sources: `docs/BRIDGE_CLASSIFICATION_METHODOLOGY.md`. One post-classification correction: Italy↔Vatican (`ITA007`↔`VAT001`) was review-reclassified from water-only to a **land border** (the Vatican is a ~2.7 km land enclave within Rome; the Tiber is ~0.6 km away, not the border), so the land-classified subset is **314** ADM1 water-only pairs, not the 315 first geometrically flagged (the shipped total is **333** after the river-gap and wide-river overlays below). ITA↔VAT is therefore pericoupled under every `coupling_standard`.
...
- **Lake overlay (uniform water policy).** The geometry build's lake filter
  ...
  candidate set, each with an OSM + web + manual bridge review) as edges, so
  ...
  counts: ADM1 **8,450** edges (8,232 moderate / 8,117 stringent), ADM0
  **326** pairs (323 moderate / 305 stringent), water-only **333** ADM1
  (115/218) + **21** ADM0 roll-ups.
...
- **Snapping-tolerance false positives (manually removed).** The snap tolerance
  ...
  **`MLT002`/`MLT019`** (Balzan / Iklin, Malta; polygons ~31 m apart) — is
  ...
  13, the river-gap overlay 6, and the lake overlay 63, for the shipped total
  of **8,450**; see the overlay sections above). A
