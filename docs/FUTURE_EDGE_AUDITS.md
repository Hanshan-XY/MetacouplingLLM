...
1. **LBR006<->LBR014** — REMOVED (added to the denylist, now `data/denylist_pairs.csv`, Stage 4).
2. **VEN001<->VEN003** — REMOVED (added to the denylist, now `data/denylist_pairs.csv`, Stage 4).
...
1. **CAN003<->CAN006** — REMOVED (added to the denylist, now `data/denylist_pairs.csv`, Stage 4).
2. **COD009<->UGA102** — REMOVED (added to the denylist, now `data/denylist_pairs.csv`, Stage 4).
3. **TZA016<->UGA040** — REMOVED (added to the denylist, now `data/denylist_pairs.csv`, Stage 4).
...
- KHM000<->KHM004 (72.7 km) and KHM000<->KHM006 (97.3 km) — Tonle Sap
  ...
  shipped as water-only rows (then in the rescreen-water manifest, now in
  `data/water_classification_pairs.csv`) with identity-audit provenance
