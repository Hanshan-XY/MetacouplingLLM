...
def test_methods_adjacency_doc_counts_match_live_data():
    ...
    regeneration) breaks the build until the methodology doc is updated too.
    """
...
def test_pinned_inputs_are_documented():
    """Every pin in build_all.py must be recorded in both docs.

    ``build_all.py --full`` verifies every pinned input's SHA-256 before
    regenerating, so a pin changed in one place and not the others leaves a
    reader unable to check the inputs -- silently, because CI runs pytest only.
    PROVENANCE.md carries the full 64-char digest; docs/REPRODUCING.md an
    abbreviated form that starts with the first 8 characters.
    """
    ...
    reproducing = REPRODUCING_MD.read_text(encoding="utf-8")
    assert set(build_all.PINNED_SHA256) == {"adm1_gpkg", "adm0_gpkg", "ocean_gpkg", "ndlsa_gpkg"}
    for key, pin in build_all.PINNED_SHA256.items():
        assert pin in provenance, f"data/PROVENANCE.md does not carry the {key} pin {pin}"
        assert f"`{pin[:8]}…" in reproducing, f"docs/REPRODUCING.md does not carry the {key} pin"
