...
These are the minimal, fast guards that would have caught it, and no more:
  ...
  * ``test_unit_merge_is_exactly_the_reviewed_pair`` -- the reviewed unit
    merge must stay exactly the maintainer-decided merge, and the denylist must
    not come back into the build (it is applied in Stage 4 from
    ``data/denylist_pairs.csv``; tested in test_apply_overlays).  It imports
    the module, so it also catches import-time errors that ``ast.parse``
    cannot see.
...
class TestBuildPericouplingDb:
    ...
    def test_unit_merge_is_exactly_the_reviewed_pair(self):
        """The reviewed unit-merge map is exactly RUS050->RUS024, and the
        geometry build carries no denylist: the five non-adjacent contacts are
        removed once, in Stage 4, from data/denylist_pairs.csv.
        ...
        assert not hasattr(mod, "_ADM1_FALSE_POSITIVE_DENYLIST")
